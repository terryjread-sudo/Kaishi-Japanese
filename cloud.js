...
(() => {
 ...
 const AVATARS=[
  ...
  {key:'journey-girl',name:'Sakura Guide',streak:5,asset:'journey-girl-avatar.png'},{key:'journey-boy',name:'Lake Explorer',streak:20,asset:'journey-boy-avatar.png'},{key:'journey-friend',name:'Cherry Friend',streak:30,asset:'journey-friend-avatar.png'},
 ...
 const announceOnboardingReady=()=>window.dispatchEvent?.(typeof Event==='function'?new Event('kaishi-onboarding-ready'):{type:'kaishi-onboarding-ready'});
 ...
 const avatarImage=(key=selectedAvatar,streak=0)=>{const definition=avatarDefinition(key);return definition?.asset?`media/profiles/${definition.asset}?v=${APP_VERSION}`:`media/profiles/${avatarKey(key)}-${avatarState(streak)}.webp?v=${APP_VERSION}`};
 const avatarUnlocked=(key,stats=adapter()?.stats?.()||{})=>{const definition=avatarDefinition(key);return Boolean(adapter()?.isTestMode?.()||(!definition?.streak&&Number(stats.mastered||0)>=Number(definition?.mastered||0))||(definition?.streak&&((stats.avatarUnlocks||[]).includes(key)||Number(stats.streak||0)>=definition.streak)))};
 const nextAvatarUnlock=(stats=adapter()?.stats?.()||{})=>AVATARS.find(avatar=>avatar.streak&&!avatarUnlocked(avatar.key,stats));
 ...
 function renderAvatarPicker(){const picker=$('#avatarPicker');if(!picker)return;const grid=picker.querySelector('.avatar-grid');if(grid&&!grid.querySelector('[data-avatar="journey-girl"]'))grid.insertAdjacentHTML('beforeend',`<button type="button" data-avatar="journey-girl"><img src="media/profiles/journey-girl-avatar.png" alt="Sakura Guide"><span>Sakura Guide</span><small data-avatar-status>Unlock at 5 rhythm days</small></button><button type="button" data-avatar="journey-boy"><img src="media/profiles/journey-boy-avatar.png" alt="Lake Explorer"><span>Lake Explorer</span><small data-avatar-status>Unlock at 20 rhythm days</small></button><button type="button" data-avatar="journey-friend"><img src="media/profiles/journey-friend-avatar.png" alt="Cherry Friend"><span>Cherry Friend</span><small data-avatar-status>Unlock at 30 rhythm days</small></button>`);const stats=adapter()?.stats?.()||{};picker.disabled=false;picker.querySelector('p').textContent=user?'Reach 5, 20 and 30 rhythm days to permanently unlock new Journey characters. Losing a streak never relocks them.':'Your guest learner is ready now. Sign in to choose characters; Journey characters unlock through permanent rhythm milestones.';picker.querySelectorAll('[data-avatar]').forEach(button=>{const definition=avatarDefinition(button.dataset.avatar),unlocked=avatarUnlocked(button.dataset.avatar,stats),chosen=Boolean(user)&&button.dataset.avatar===selectedAvatar;button.classList.toggle('selected',chosen);button.classList.toggle('locked',!unlocked);button.setAttribute('aria-pressed',String(chosen));button.setAttribute('aria-disabled',String(!user||!unlocked));const status=button.querySelector('[data-avatar-status]');if(status)status.textContent=unlocked?(user?'Available':'Sign in to choose'):definition?.streak?`Unlock at ${definition.streak} rhythm days`:`Master ${definition?.mastered||0} words`;button.setAttribute('aria-label',`${definition?.name||'Character'}. ${unlocked?'Unlocked':`Unlocks at ${definition?.streak?`${definition.streak} rhythm days`:`${definition?.mastered||0} mastered words`}`}${chosen?'. Selected':''}`)})}
...
async function loadLeaderboard(){
 ...
 async function handleSession(session){user=session?.user||null;window.dispatchEvent(new CustomEvent('kaishi-auth-change',{detail:{signedIn:Boolean(user),userId:user?.id||null}}));if(!user){if(adapter()?.profileId?.()&&adapter()?.profileId?.()!=='guest'){adapter()?.activateProfile?.('guest');announceOnboardingReady(false);return}renderSignedOut();await loadLeaderboard();announceOnboardingReady(false);return}if(!adapter()?.isTestMode?.()&&adapter()?.profileId?.()!==user.id){adapter()?.activateProfile?.(user.id);return}renderStudioAccess();await loadEmailPreferences();if(isOwner()&&!adminUsersLoaded)loadAdminUsers();if(adapter()?.isTestMode?.()){const{data:entry}=await client.from('leaderboard_entries').select('*').eq('user_id',user.id).maybeSingle();renderSignedIn(entry||{});setStatus('Test learner is isolated. Cloud sync is paused.','ok');announceOnboardingReady(true);return}if(initialisedUserId===user.id){announceOnboardingReady(true);return}initialisedUserId=user.id;await initialiseAccount();await initialiseFriends();await redeemFriendInviteFromUrl();announceOnboardingReady(true)}
 ...
 async function init(){
  ...
  if(!config?.url||!config?.publishableKey||!sdk?.createClient){renderSignedOut('Cloud sync could not start. Guest mode is still available.');setStatus('Cloud configuration or library is unavailable.','error');announceOnboardingReady(false);return}
  ...
  const{data,error}=await client.auth.getSession();if(error){renderSignedOut();setStatus(describeError(error),'error');announceOnboardingReady(false)}else await handleSession(data.session);
