...
(() => {
  ...
  function showDenied(title,message){
    ...
    const back=document.createElement('a');back.className='back-to-app';back.href='index.html';back.textContent='← Back to Kaishi Japanese';
  ...
  async function checkAccess(){
    if(!config?.url||!config?.publishableKey||!sdk?.createClient){
      showDenied('Access check unavailable','Kaishi Japanese could not verify the signed-in account.');return;
    }
    ...
    if(error){showDenied('Access check failed','Return to Kaishi Japanese and sign in again.');return}
    ...
    showDenied('Owner sign-in required',`Return to Kaishi Japanese and sign in as @${OWNER_LOGIN} to open Mnemonic Studio.`);
  }

  checkAccess().catch(()=>showDenied('Access check failed','Return to Kaishi Japanese and sign in again.'));
})();
