...
(() => {
  ...
  function renderArchivedLessonCard(data) {
    ...
    const total = lessonCount();
    const current = currentChapter();
    const visible = new Set(data.filter(item => item.type === 'past' && Number.isFinite(item.chapter)).map(item => item.chapter));
    ...
    host.innerHTML = `<div class="journey-archive-node"><div class="journey-archive-marker" aria-hidden="true">◷</div><div class="journey-archive-card"><div class="journey-archive-heading"><strong>Past lessons</strong><button type="button" class="journey-archive-toggle" aria-expanded="${expanded}" aria-controls="journeyArchivedLessonBody" title="${expanded ? 'Collapse' : 'Expand'} past lessons">${expanded ? '⌃' : '⌄'}<span class="sr-only">${expanded ? 'Collapse' : 'Expand'} past lessons</span></button></div><div id="journeyArchivedLessonBody" class="journey-archive-body"${expanded ? '' : ' hidden'}><label><span class="sr-only">Choose a past lesson</span><select id="journeyArchivedLessonSelect" aria-label="Choose a past lesson">${archived.map(item => `<option value="${item.chapter}"${item.chapter === selected.chapter ? ' selected' : ''}>${esc(item.title)} · ${Number(item.detail.match(/(\d+)%/)?.[1] || 0)}% strength</option>`).join('')}</select></label><p class="journey-archive-summary"><strong>${esc(selected.title)}</strong><span>${esc(selected.detail)} · ${words.length}/${stats.words.length} words introduced</span></p><div id="journeyArchivedLessonDetails" hidden></div><div class="journey-archive-actions"><button type="button" data-archive-results>View lesson results</button><button type="button" class="primary" data-archive-practice>Practice lesson</button></div></div></div></div>`;
  ...
  function renderExperimentalTimeline(data, track, options = {}) {
    track._kqExperimentalResizeObserver?.disconnect?.();
    ...
    const ITEM_GAP = 12;
    const BUFFER = 5;
    const heightCache = track._kqExperimentalHeightCache || (track._kqExperimentalHeightCache = {});
    ...
    const heightKey = item => `${item.id}:${item.id === selected.id ? 'expanded' : 'compact'}`;
    ...
    track.innerHTML = `<div class="experimental-journey-shell"><img class="experimental-journey-hero-art" src="media/experimental/kaishi-journey-hero.png" alt="" aria-hidden="true">
      <button type="button" class="experimental-journey-settings" data-experimental-settings-trigger aria-label="Open settings">⚙</button>
      <section class="experimental-journey-hero-copy" aria-label="Kaishi Japanese"><div><span class="experimental-hero-brand">Kaishi <em>Japanese</em> <small class="experimental-hero-version">v${APP_VERSION}</small></span><p>一歩ずつ、もっと遠くへ</p><small>A brighter you,<br>a closer Japan.</small></div><div class="experimental-hero-actions"><button type="button" class="experimental-hero-profile" data-experimental-profile-trigger aria-haspopup="dialog"><img class="experimental-journey-profile-avatar" src="media/profiles/guest-learner.webp" alt="Guest learner"><span><b class="experimental-journey-profile-name">Guest learner</b><small lang="ja">はじめよう！</small></span><strong aria-hidden="true">›</strong></button><button type="button" class="experimental-hero-japan-ready" data-experimental-japan-ready><svg class="experimental-hero-japan-icon" viewBox="0 0 48 48" aria-hidden="true"><path d="M8 17h32M12 17l3-8 3 8m12 0 3-8 3 8M16 18v21m16-21v21M10 39h28M23 17v22"/></svg><span>Japan Ready</span><strong aria-hidden="true">›</strong></button></div></section>
    ...
    const bindWindowEvents = () => {
      virtualWindow.querySelectorAll('[data-experimental-select]').forEach(node => {
        const selectNode = () => {
    ...
    const renderWindow = (centerSelection = false) => {
      ...
      // Keep a useful first viewport mounted even before flex layout has
      // reported its final height. This avoids an underfilled timeline on
      // mobile and gives scroll/resize observers enough rows to settle.
      visibleEnd = Math.max(visibleEnd, Math.min(all.length, visibleStart + 8));
      ...
      scheduleCardStyles();
      const jump = track.querySelector('[data-experimental-jump]');
      const updateJumpVisibility = () => {
        const current = virtualWindow.querySelector(`[data-virtual-index="${currentIndex}"]`), viewport = timeline.getBoundingClientRect(), rect = current?.getBoundingClientRect();
        if (jump) jump.hidden = Boolean(rect && rect.top >= viewport.top && rect.bottom <= viewport.bottom);
      };
      ...
      const observer = new ResizeObserver(entries => {
        ...
        entries.forEach(entry => {
          const row = entry.target, item = all[Number(row.dataset.virtualIndex)];
          if (!item) return;
          const measured = Math.ceil(entry.borderBoxSize?.[0]?.blockSize || entry.contentRect.height);
          const key = heightKey(item);
          if (measured > 0 && Math.abs(Number(heightCache[key] || 0) - measured) > 1) { heightCache[key] = measured; changed = true; }
        });
        if (changed) requestAnimationFrame(() => renderExperimentalTimeline(data, track, { preserveScrollTop: timeline.scrollTop }));
      });
      virtualWindow.querySelectorAll('.experimental-timeline-item').forEach(row => observer.observe(row));
      track._kqExperimentalResizeObserver = observer;
      timeline.addEventListener('scroll', updateJumpVisibility, { passive: true });
    };
