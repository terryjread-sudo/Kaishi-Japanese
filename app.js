...
function renderLearningRhythmWeek(){
 ...
 const japaneseWeekdays=['月','火','水','木','金','土','日'];days.innerHTML=Array.from({length:7},(_,offset)=>{const date=new Date(monday);date.setDate(monday.getDate()+offset);const key=day(date),entry=history[key],label=date.toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric'});return `<span class="learning-rhythm-week-day${entry?' completed':''}${key===todayKey?' today':''}" aria-label="${esc(label)}${entry?', learning activity completed':''}" title="${esc(entry?`${label} · ${entry.source||'Learning activity'}`:label)}"><small lang="ja" aria-hidden="true">${japaneseWeekdays[offset]}</small><b>${date.getDate()}</b>${entry?'<i aria-hidden="true"></i>':''}</span>`}).join('');
...
function lessonMasteryPanelHTML(itemIndex){const snapshot=lessonMasterySnapshot(itemIndex),stages=[['learned','Lesson complete'],['strengthening','Strengthening'],['confident','Confident'],['mastered','Durable mastery']],activeIndex=Math.max(0,stages.findIndex(([id])=>id===snapshot.stage));return`<aside class="lesson-mastery-path" style="--lesson-strength:${snapshot.strength}%" aria-label="Sensei’s path: ${esc(snapshot.label)}. Current strength ${snapshot.strength}%."><div class="lesson-mastery-heading"><strong>Sensei’s Path: ${esc(snapshot.label)} <em>(${snapshot.strength}%)</em></strong></div><div class="lesson-mastery-stages">${stages.map(([id,label],stageIndex)=>`<span class="${stageIndex<=activeIndex?'reached':''} ${stageIndex===activeIndex?'current':''}"><i>${stageIndex<activeIndex?'✓':stageIndex+1}</i>${esc(label)}</span>`).join('')}</div><details><summary>How this works</summary><p>${esc(snapshot.recommendation)} Completing a lesson opens the next one; it never requires perfect recall. Focused practice strengthens recognition, listening and recall. Durable mastery is earned when you retrieve the words again after a recall gap.</p></details></aside>`}
