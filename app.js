...
function lessonStrengthForWord(word){const skills=progress[word?.id]?.skills||{},values=Object.values(skills).map(metric=>Number(metric?.strength||0)).filter(value=>value>0);return values.length?Math.round(values.reduce((sum,value)=>sum+value,0)/values.length*100):wordIntroduced(word)?0:0}
