...
function sessionSentencePlan(words){const fresh=words.filter(word=>Number(pFor(word.id).stage||0)===0);if(fresh.length<2)return null;for(const target of fresh){const sentence=target.sentence||target.exampleSentence||'',companion=fresh.find(word=>word!==target&&sentence.includes(word.word));if(!sentence||!companion)continue;const chunks=sentenceGuideParts(sentence).map(part=>part.text).filter(Boolean);if(chunks.length>1)return{target,companion,sentence,meaning:target.sentenceMeaning||target.exampleSentenceMeaning||'',chunks}}return null}
