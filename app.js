...
function wordBoundaryMatch(sentence,word){if(!sentence||!word)return false;const isHan=ch=>/[\u4e00-\u9fff\u3400-\u4dbf]/.test(ch||'');let idx=sentence.indexOf(word);while(idx!==-1){const before=sentence[idx-1],after=sentence[idx+word.length];if(!isHan(before)&&!isHan(after))return true;idx=sentence.indexOf(word,idx+1)}return false}
function pickLinkedNewWords(pool,count){const unseenPool=pool.filter(v=>!progress[v.id]||Number(progress[v.id].stage||0)===0);if(count<=0||!unseenPool.length)return unseenPool.slice(0,count);for(const word of unseenPool){const sentence=word.sentence||word.exampleSentence||'';if(!sentence)continue;const companion=unseenPool.find(other=>other!==word&&wordBoundaryMatch(sentence,other.word));if(companion){const rest=unseenPool.filter(w=>w!==word&&w!==companion).slice(0,Math.max(0,count-2));return[word,companion,...rest].slice(0,count)}}return unseenPool.slice(0,count)}
function sessionSentencePlan(words){const fresh=words.filter(word=>Number(pFor(word.id).stage||0)===0);if(fresh.length<2)return null;for(const target of fresh){const sentence=target.sentence||target.exampleSentence||'',companion=fresh.find(word=>word!==target&&wordBoundaryMatch(sentence,word.word));if(!sentence||!companion)continue;const chunks=sentenceGuideParts(sentence).map(part=>part.text).filter(Boolean);if(chunks.length>1)return{target,companion,sentence,meaning:target.sentenceMeaning||target.exampleSentenceMeaning||'',chunks}}return null}
function renderSessionSentenceBridge(item){const plan=item.sentencePlan,chosen=[];const draw=()=>{const remaining=plan.chunks.map((chunk,index)=>({chunk,index})).filter(item=>!chosen.includes(item.index));$('#card').innerHTML=`${senseiBlock(`You met ${plan.target.word} and ${plan.companion.word} together. Build their real sentence from its meaning, then notice how the connector words hold it together.`)}<div class="eyebrow">Meaning-first sentence bridge</div><h2>${esc(plan.meaning||'Build this complete thought')}</h2><p class="sentence-bridge-hint">Use every Japanese chunk once. です means “is / are” and politely completes many statements.</p><div class="sentence-bridge-answer" lang="ja">${chosen.length?chosen.map(index=>`<span>${esc(plan.chunks[index])}</span>`).join(''):'<span class="muted">Build the Japanese sentence here</span>'}</div><div class="sentence-bridge-choices">${shuffle(remaining).map(({chunk,index})=>`<button type="button" data-sentence-chunk="${index}" lang="ja">${esc(chunk)}</button>`).join('')}</div>${chosen.length===plan.chunks.length?'<button id="checkSentenceBridge" class="primary reveal">Check my sentence</button>':''}`;document.querySelectorAll('[data-sentence-chunk]').forEach(button=>button.onclick=()=>{chosen.push(+button.dataset.sentenceChunk);draw()});const check=$('#checkSentenceBridge');if(check)check.onclick=()=>{const correct=chosen.every((value,index)=>value===index),answer=$('.sentence-bridge-answer');answer.classList.add(correct?'correct':'wrong');const feedback=document.createElement('p');feedback.className=correct?'ok':'guess-feedback';feedback.textContent=correct?'Excellent — the words work together in a complete Japanese thought.':`The natural order is: ${plan.sentence}`;answer.after(feedback);check.disabled=true;setTimeout(()=>grade(plan.target,'sentence',correct?3:1,correct),1200)}};draw()}
