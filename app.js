...
function renderLearningRhythmWeek(){
 ...
 days.innerHTML=Array.from({length:7},(_,offset=>{const date=new Date(monday);date.setDate(monday.getDate()+offset);const key=day(date),entry=history[key],label=date.toLocaleDateString(undefined,{weekday:'long',month:'long',day:'numeric'});return `<span class="learning-rhythm-week-day${entry?' completed':''}${key===todayKey?' today':''}" aria-label="${esc(label)}${entry?', learning activity completed':''}" title="${esc(entry?`${label} · ${entry.source||'Learning activity'}`:label)}"><small>${date.toLocaleDateString(undefined,{weekday:'short'})}</small><b>${date.getDate()}</b>${entry?'<i aria-hidden="true"></i>':''}</span>`})).join('');
