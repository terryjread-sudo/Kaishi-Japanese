...
function finishKotobaEcho(){const run=kotobaEchoRun;if(!run)return;const firstCount=run.firstRound.length,missed=run.missed.size;meta.kotobaEchoHistory=[...(Array.isArray(meta.kotobaEchoHistory)?meta.kotobaEchoHistory:[]),{at:Date.now(),words:firstCount,correct:run.correct,missed}].slice(-50);save();updateHome();const card=$('#kotobaEchoCard');card.innerHTML=`<section class="kotoba-echo-summary"><span class="kotoba-echo-microphone" aria-hidden="true">🎤</span><span class="eyebrow">Encore complete</span><h2>${run.correct} of ${firstCount} words recognised</h2><p>${missed?'You heard the model for every missed word, then gave each one a second chance.':'A clean karaoke run. Your production practice has been recorded.'}</p><button id="kotobaEchoDone" class="primary" type="button">Return to Journey</button></section>`;$('#kotobaEchoDone').onclick=()=>{kotobaEchoRun=null;openJourney('missions')}}
function kotobaEchoNormalise(value=''){return String(value).replace(/[\s、。！？!?.]/g,'')}
function finishKotobaEchoRecognition(word,transcript,error=''){const run=kotobaEchoRun;if(!run||run.wordId!==word.id)return;clearTimeout(kotobaEchoStopTimer);kotobaEchoStopTimer=0;kotobaEchoRecognition=null;run.listening=false;run.wordId='';const heard=kotobaEchoNormalise(transcript),forms=[word.word,word.reading].map(kotobaEchoNormalise).filter(Boolean),matched=Boolean(heard&&forms.some(form=>heard.includes(form)));if(!heard){renderKotobaEchoFeedback(word,'retry');return}if(matched){run.correct++;kotobaEchoStrengthen(word);renderKotobaEchoFeedback(word,'correct');return}if(run.phase==='first')run.missed.add(word.id);renderKotobaEchoFeedback(word,'wrong')}
