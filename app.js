...
async function launchAdminTestActivity(id){
 ...
 if(id!=='colosseum'&&id!=='kotobaEcho'&&!PATH_MILESTONES.some(item=>item.id===id)){toast('Choose a valid immersive activity');return false}
...
async function init(){
 ...
 try{
  [vocab,kanaData,mangaStories,conversations,theatreScenes,grammarLessons,componentData,memoryScenes,ankiContent,topicData,learningGraph,katakanaCore,vocabularyOrthography,storySceneCatalog]=await Promise.all([
