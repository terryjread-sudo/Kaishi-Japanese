...
function renderLessonStoryScene(item){
 ...
 $('#card').innerHTML=`${senseiBlock('Aiko and Kai are using today’s Japanese in a real little moment. Listen first, then notice who does what.')}<div class="eyebrow">Aiko & Kai story scene</div><section class="lesson-story-scene"><div class="lesson-story-art">${image?`<img src="${esc(image)}" alt="A mnemonic scene for Aiko and Kai’s sentence">`:''}<div class="lesson-story-cast"><span><img src="media/profiles/girl-base.webp" alt="">Aiko · アイコ</span><span><img src="media/profiles/boy-base.webp" alt="">Kai · カイ</span></div></div><div class="lesson-story-copy"><span class="lesson-story-focus">Notice: ${esc(story.grammarFocus)}</span><p class="lesson-story-sentence" lang="ja">${esc(story.sentence)}</p><small>${esc(story.reading)}</small><p>${esc(story.meaning)}</p><button id="storySentenceAudio" class="audio" type="button">🔊 Hear Aiko and Kai</button></div></section><aside class="lesson-story-note"><b>Why it works</b><p>${esc(story.grammarNote)}</p></aside><section class="lesson-story-check"><span>Quick scene check</span><h2>${esc(story.question.prompt)}</h2><div class="choices">${shuffle(story.question.choices).map(choice=>`<button class="choice" data-answer="${encodeURIComponent(choice)}" type="button">${esc(choice)}</button>`).join('')}</div><div id="storySentenceFeedback" class="teacher-answer-feedback" hidden aria-live="polite"></div></section>`;
 $('#storySentenceAudio').onclick=()=>speakJapanese(story.sentence,null,'kai');
 bindChoices(story.question.answer,'sentence',(ok)=>{const feedback=$('#storySentenceFeedback');if(!feedback)return;feedback.hidden=false;feedback.innerHTML=`<p class="${ok?'ok':'guess-feedback'}">${ok?'Exactly. You read the scene correctly.':`Not quite. ${esc(story.question.explanation)}`}</p>${ok?'':`<p lang="ja"><b>${esc(story.sentence)}</b></p><button id="storyAnswerAudio" class="audio" type="button">🔊 Hear the model sentence</button>`}`;const replay=$('#storyAnswerAudio');if(replay)replay.onclick=()=>speakJapanese(story.sentence,null,'kai');if(!ok)speakJapanese(story.sentence,null,'kai')});
}
