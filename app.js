...
function bindChoices(answer,skill,onReveal){document.querySelectorAll('.choice').forEach(b=>b.onclick=()=>{if(revealed)return;revealed=true;const val=decodeURIComponent(b.dataset.answer),ok=val===answer,v=current.v;b.classList.add(ok?'correct':'wrong');document.querySelectorAll('.choice').forEach(x=>{if(decodeURIComponent(x.dataset.answer)===answer)x.classList.add('correct');x.disabled=true});if(onReveal)onReveal(ok);$('#hintBtn')?.remove();const overlay=document.createElement('section');overlay.className=`answer-result-overlay ${ok?'correct':'wrong'}`;overlay.setAttribute('role','status');overlay.setAttribute('aria-live','polite');overlay.innerHTML=`<div class="answer-result-icon">${ok?'✓':'✕'}</div><div class="answer-result-copy"><strong>${ok?'Correct!':'Not quite'}</strong><span>The correct answer is <b>${esc(answer)}</b></span></div>${v.wordAudio?'<button id="choiceAnswerAudio" class="audio answer-result-audio" aria-label="Play the correct Japanese answer" title="Play the correct Japanese answer">🔊</button>':''}<button id="choiceAnswerContinue" class="primary answer-result-continue">Continue →</button>`;$('#card').appendChild(overlay);const audio=$('#choiceAnswerAudio');if(audio)audio.onclick=()=>play(v.wordAudio);$('#choiceAnswerContinue').onclick=()=>grade(v,skill,ok?(hintUsed?2:3):1,ok);overlay.scrollIntoView({behavior:'smooth',block:'nearest'});$('#choiceAnswerContinue').focus({preventScroll:true})})}
