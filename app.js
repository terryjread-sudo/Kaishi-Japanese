...
const CLOUD_PROFILE_KEY='kq-active-local-profile-v1',CLOUD_PROFILE_MIGRATION_KEY='kq-profile-storage-migrated-v1',GUEST_PROFILE_ID='guest';
let activeLocalProfileId=GUEST_PROFILE_ID;
function scopedProfileStorageKey(profileId,key){return`kq-profile-v1:${profileId}:${key}`}
function migrateLegacyProfileStorage(){
 try{
  if(localStorage.getItem(CLOUD_PROFILE_MIGRATION_KEY)==='1')return;
  ['kq-progress','kq-settings','kq-meta','kq-resumable-mission','kq-mastery-challenges-v1'].forEach(key=>{const value=localStorage.getItem(key),target=scopedProfileStorageKey(GUEST_PROFILE_ID,key);if(value!==null&&localStorage.getItem(target)===null)localStorage.setItem(target,value);if(value!==null)localStorage.removeItem(key)});
  localStorage.setItem(CLOUD_PROFILE_MIGRATION_KEY,'1');
 }catch{}
}
migrateLegacyProfileStorage();
try{activeLocalProfileId=localStorage.getItem(CLOUD_PROFILE_KEY)||GUEST_PROFILE_ID}catch{}
const profileStorageKey=key=>isAdminTestMode()?key.replace(/^kq-/,'kq-admin-test-'):scopedProfileStorageKey(activeLocalProfileId,key);
...
let vocab=[], kanaData=[], mangaStories=[], conversations=[], theatreScenes=[], grammarLessons=[], componentData={components:{},kanji:[]}, ankiContent={records:[]}, topicData={topics:[]}, katakanaCore={records:[]}, vocabularyOrthography={entries:{}}, learningGraph={regions:[],topics:[],foundations:[],assignmentsByWord:{}}, storySceneCatalog={schemaVersion:1,scenes:[]}, session=[], index=0, current=null, revealed=false, startedAt=0, hintUsed=false, memoryScenes={};
let deferredContentReady=false, deferredContentPromise=null;
...
const META_DEFAULTS={lastStudy:'',streak:0,avatarUnlocks:[],totalAnswers:0,totalCorrect:0,kanaAnswers:0,kanaCorrect:0,grammarAnswers:0,grammarCorrect:0,kanaProgress:{},grammarProgress:{},connectorProgress:{},sentenceLabProgress:{lessons:{},saved:[],mistakes:[],totalAnswers:0,totalCorrect:0},notebook:{words:[]},notebookStarHintSeen:false,mangaProgress:{},conversationProgress:{},theatreProgress:{},deviceRepairProgress:{schemaVersion:2,devices:{},workshop:{schemaVersion:1,xp:0,level:1,selectedSkin:'bench',unlockedSkins:['bench'],unlockedModules:['standard'],soundPack:'mechanical',updatedAt:0}},canDoAwards:[],pathUnlocks:[],pathVisits:{},pathOverrides:[],chapterOverrides:[],dailyJourneyRoute:null,journeyCurriculumRevision:'',dailyActivity:null,unlockNoticesSeen:[],unlockNoticesDismissed:[],activityPurchases:['vocabulary','kana','sentenceLab'],adventurePointsSpent:0,rhythm:{behind:0,lastChecked:'',quickWeek:'',quickUsed:0,cycles:0},rhythmHistory:{},activityModeLevels:{},karutaSessions:[],monsterVictories:[],totalMonsterVictories:0,streakRescue:null,activeCampaign:'journey',campaignProgress:{},sessionHistory:[],kotobaEchoHistory:[],updatedAt:0};
...
meta.connectorProgress=meta.connectorProgress||{};
...
meta.avatarUnlocks=Array.isArray(meta.avatarUnlocks)?[...new Set(meta.avatarUnlocks.map(String))]:[];
...
function serialiseMissionItem(item){return{wordId:item?.v?.id||'',skill:item?.skill||'',character:item?.character,requiredAssessment:item?.requiredAssessment,pictureMode:item?.pictureMode||'',bossTopicId:item?.bossTopicId||'',storySceneId:item?.storyScene?.id||''}}
function saveMissionResume(){
 if(!session.length||index>=session.length||pictureGameActive||karutaActive||battleActive)return;
 const remaining=session.slice(current?.completed?index+1:index).map(serialiseMissionItem).filter(item=>item.wordId&&item.skill);
...
function resumableMission(){
 ...
 const restored=saved.remaining.map(item=>({v:byId.get(item.wordId),skill:item.skill,character:item.character,requiredAssessment:item.requiredAssessment,pictureMode:item.pictureMode||undefined,bossTopicId:item.bossTopicId||undefined,storyScene:item.storySceneId?window.KaishiActivityPolicy?.findLessonStoryScene?.(storySceneCatalog,item.storySceneId)||undefined:undefined})).filter(item=>item.v&&item.skill&&(!item.storySceneId||item.storyScene));
...
function normaliseNotebookState(){meta.notebook=meta.notebook||{};meta.notebook.words=Array.isArray(meta.notebook.words)?meta.notebook.words.filter(item=>item&&item.wordId).slice(0,NOTEBOOK_WORD_LIMIT):[];return meta.notebook}
function notebookWords(){return normaliseNotebookState().words}
...
function experimentalUtilityHeader(action,closeAttribute=''){
 const copy=EXPERIMENTAL_UTILITY_COPY[action],sourceIcon=$(`[data-experimental-nav="${action}"] .experimental-nav-icon`)||$('[data-collection-notebook] .experimental-nav-icon'),icon=sourceIcon?sourceIcon.outerHTML:'';
...
function renderLearningNotebook(tab='words'){
 const dialog=ensureNotebookDialog(),content=$('#learningNotebookContent'),words=notebookWords(),sentences=notebookSentences(),isWords=tab==='words',items=isWords?words:sentences;
 dialog.querySelectorAll('[data-notebook-tab]').forEach(button=>{const active=button.dataset.notebookTab===tab;button.classList.toggle('active',active);button.setAttribute('aria-selected',String(active))});
 const heading=isWords?'Saved words':'Saved sentences',empty=isWords?'Save a word from any learning card to keep it here.':'Save a useful pattern in Sentence Lab to keep it here.',entries=isWords?items.map(notebookWordHtml).join(''):items.map(notebookSentenceHtml).join('');
 content.innerHTML=`<section class="notebook-paper"><div class="notebook-paper-heading"><div><span>${items.length} saved</span><h3>${heading}</h3></div><details class="notebook-admin-menu"><summary aria-label="Notebook options">•••</summary><div><button id="notebookClearTab" type="button"${items.length?'':' disabled'}>Clear ${isWords?'words':'sentences'}</button><button id="notebookResetAll" type="button"${words.length||sentences.length?'':' disabled'}>Reset notebook</button></div></details></div>${items.length?`<div class="notebook-entry-list">${entries}</div>`:`<div class="notebook-empty"><span>📓</span><p>${empty}</p></div>`}</section>`;
 dialog.querySelectorAll('[data-notebook-tab]').forEach(button=>button.onclick=()=>renderLearningNotebook(button.dataset.notebookTab));
 const toggleDetail=(selector,className)=>content.querySelectorAll(selector).forEach(button=>button.onclick=()=>{const detail=button.closest('.notebook-entry').querySelector(className);detail.hidden=!detail.hidden});
 toggleDetail('[data-notebook-word-reading],[data-notebook-sentence-reading]','.notebook-reading');toggleDetail('[data-notebook-word-meaning],[data-notebook-sentence-meaning]','.notebook-meaning');
 content.querySelectorAll('[data-notebook-word-audio]').forEach(button=>button.onclick=()=>play(words[+button.dataset.notebookWordAudio]?.wordAudio));content.querySelectorAll('[data-notebook-sentence-audio]').forEach(button=>button.onclick=()=>speakJapanese(sentences[+button.dataset.notebookSentenceAudio]?.sentence));
 content.querySelectorAll('[data-notebook-word-remove]').forEach(button=>button.onclick=()=>{words.splice(+button.dataset.notebookWordRemove,1);save();renderLearningNotebook(tab)});content.querySelectorAll('[data-notebook-sentence-remove]').forEach(button=>button.onclick=()=>{sentences.splice(+button.dataset.notebookSentenceRemove,1);save();renderLearningNotebook(tab)});
 content.querySelectorAll('[data-notebook-sentence-practise]').forEach(button=>button.onclick=()=>{const item=sentences[+button.dataset.notebookSentencePractise];dialog.close();if(window.KaishiSentenceLab?.practiseSavedSentence)window.KaishiSentenceLab.practiseSavedSentence(item);else speakJapanese(item?.sentence)});
 $('#notebookClearTab').onclick=()=>{if(confirm(`Clear all saved ${isWords?'words':'sentences'}?`)){if(isWords)meta.notebook.words=[];else meta.sentenceLabProgress.saved=[];save();renderLearningNotebook(tab)}};$('#notebookResetAll').onclick=()=>{if(confirm('Reset your notebook? This removes every saved word and sentence.')){meta.notebook.words=[];meta.sentenceLabProgress.saved=[];save();renderLearningNotebook(tab)}};
}
function openLearningNotebook(tab='words'){const dialog=ensureNotebookDialog();renderLearningNotebook(tab);if(!dialog.open)dialog.showModal()}
function addNotebookSaveControl(word){const card=$('#card');if(!card||!word||card.querySelector('[data-notebook-save-word]'))return;const target=card.querySelector('.meet-word-written .jp,.first-encounter-word,.word-pronunciation-card strong[lang="ja"],.recall-front.jp,#answer .jp');if(!target)return;const button=document.createElement('button'),setSavedState=isSaved=>{button.classList.toggle('saved',isSaved);button.textContent=isSaved?'★':'☆';button.setAttribute('aria-pressed',String(isSaved));button.setAttribute('aria-label',`${isSaved?'Remove':'Save'} ${word.word} ${isSaved?'from':'to'} notebook`);button.title=isSaved?'Remove from notebook':'Save to notebook'};button.type='button';button.className='notebook-save-control';button.dataset.notebookSaveWord=word.id;setSavedState(notebookWordSaved(word));button.onclick=()=>setSavedState(toggleNotebookWord(word));const placement=target.matches('.first-encounter-word')?'encounter':target.matches('.meet-word-written .jp')?'meet':target.matches('.word-pronunciation-card strong')?'pronunciation':'recall',row=document.createElement('span');row.className=`notebook-word-row notebook-word-row--${placement}`;target.before(row);row.append(target,button);if(!meta.notebookStarHintSeen){meta.notebookStarHintSeen=true;save();const hint=document.createElement('aside');hint.className='notebook-star-hint';hint.innerHTML='<span aria-hidden="true">☆</span><p>Tap the star to save this word to your Notebook.</p><button type="button">Got it</button>';hint.querySelector('button').onclick=()=>hint.remove();row.after(hint)}}
window.KaishiNotebook={open:openLearningNotebook,isWordSaved:notebookWordSaved,toggleWord:toggleNotebookWord,words:()=>structuredClone(notebookWords()),sentences:()=>structuredClone(notebookSentences())};
...
function syncExperimentalHeaderClearance(){const style=document.body?.style;if(!style||typeof style.setProperty!=='function')return;const header=$('#appHeader.experimental-journey-enabled'),hidden=document.body.classList.contains('experimental-settings-active')||document.body.classList.contains('experimental-japan-ready-active');style.setProperty('--experimental-header-clearance',header&&!hidden?`${Math.ceil(header.getBoundingClientRect().height+16)}px`:'')}
...
function unlockStreakAvatars(){const streak=Number(meta.streak||0),milestones=[[5,'journey-girl'],[20,'journey-boy'],[30,'journey-friend']];meta.avatarUnlocks=Array.isArray(meta.avatarUnlocks)?meta.avatarUnlocks:[];milestones.forEach(([days,key])=>{if(streak>=days&&!meta.avatarUnlocks.includes(key))meta.avatarUnlocks.push(key)})}
...
function awardRhythmDay(quick=false,source='learning activity'){const rhythm=reconcileRhythm(),today=day(),gap=dateGap(meta.lastStudy,today);if(meta.lastStudy!==today){meta.streak=Number(meta.streak||0)>0&&gap<=3?Number(meta.streak)+1:1;meta.lastStudy=today}stampLearningRhythmDay(source);if(quick)rhythm.quickUsed=Math.min(2,rhythm.quickUsed+1);else if(rhythm.behind>0)rhythm.behind--;rhythm.lastChecked=today;unlockStreakAvatars();return rhythm}
...
function renderAdminTestMode(){
 ...
 const collapse=$('#adminTestCollapse'),controls=$('#adminTestControls');
 if(collapse&&controls){const collapsed=sessionStorage.getItem('kq-admin-test-collapsed')==='1';controls.hidden=collapsed;collapse.setAttribute('aria-expanded',String(!collapsed));collapse.textContent=collapsed?'Expand':'Collapse';collapse.onclick=()=>{const next=!controls.hidden;controls.hidden=next;sessionStorage.setItem('kq-admin-test-collapsed',next?'1':'0');collapse.setAttribute('aria-expanded',String(!next));collapse.textContent=next?'Expand':'Collapse';};}
 if(isAdminTestMode()){
...
window.KaishiJapanReadyBridge={version:3,getVocab:()=>vocab,getProgress:()=>progress,getMeta:()=>meta,save:()=>save(),show,updateHome,isTestMode:isAdminTestMode,playWord:word=>speak(word?.word||word?.reading||''),startFocusedStudy:wordIds=>{const ids=[...new Set(wordIds)].filter(Boolean);if(!ids.length){toast('No matching study words were found yet');return}activityReturnScreen='japanReady';makeTargetedMasterySession(ids,'travel')},wordIntroduced,wordMastery:word=>mastery(progress[word.id]),stats:()=>({started:started(),accuracy:accuracy(),mastered:Object.values(progress).filter(mastery).length})};
...
window.KaishiQuestCloudAdapter={
 ...
 cloudSnapshot:()=>isAdminTestMode()?null:{version:4,progress,meta},
 profileId:()=>activeLocalProfileId,
 profileSnapshot:profileId=>({version:3,progress:loadJSON(scopedProfileStorageKey(profileId,'kq-progress'),{}),meta:{...META_DEFAULTS,...loadJSON(scopedProfileStorageKey(profileId,'kq-meta'),{})},settings:{...defaults,...loadJSON(scopedProfileStorageKey(profileId,'kq-settings'),{})}}),
 activateProfile:profileId=>{if(!profileId||profileId===activeLocalProfileId)return false;save(false);try{localStorage.setItem(CLOUD_PROFILE_KEY,profileId);location.reload();return true}catch{return false}},
 restore:data=>{const localDailyReviewPlan=meta.dailyReviewPlan&&meta.dailyReviewPlan.date===day()?structuredClone(meta.dailyReviewPlan):null;progress=data?.progress||{};meta={...META_DEFAULTS,...(data?.meta||{})};meta.kanaProgress=meta.kanaProgress||{};meta.grammarProgress=meta.grammarProgress||{};meta.sentenceLabProgress={lessons:{},saved:[],mistakes:[],totalAnswers:0,totalCorrect:0,...(meta.sentenceLabProgress||{})};normaliseNotebookState();meta.mangaProgress=meta.mangaProgress||{};meta.conversationProgress=meta.conversationProgress||{};meta.theatreProgress=meta.theatreProgress||{};meta.pathUnlocks=Array.isArray(meta.pathUnlocks)?[...meta.pathUnlocks]:[];meta.pathVisits={...(meta.pathVisits||{})};meta.pathOverrides=Array.isArray(meta.pathOverrides)?[...meta.pathOverrides]:[];meta.chapterOverrides=Array.isArray(meta.chapterOverrides)?[...meta.chapterOverrides]:[];const legacy=meta.rhythm||meta.garden||{};meta.rhythm={...META_DEFAULTS.rhythm,behind:Number(legacy.behind||0),lastChecked:String(legacy.lastChecked||''),quickWeek:String(legacy.quickWeek||''),quickUsed:Number(legacy.quickUsed||0),cycles:Number(legacy.cycles||0)};delete meta.garden;delete meta.gardenApSpent;rhythmState();if(localDailyReviewPlan&&(!meta.dailyReviewPlan||meta.dailyReviewPlan.date!==day()||Number(localDailyReviewPlan.initialTotal||0)>=Number(meta.dailyReviewPlan.initialTotal||0)))meta.dailyReviewPlan=localDailyReviewPlan;settings={...defaults,...settings,...(data?.settings||{}),playMode:'journey'};save(false);if($('#pictureDifficulty')){$('#pictureDifficulty').value=settings.pictureDifficulty;$('#mnemonicStyle').value=settings.mnemonicStyle;$('#autoAudio').checked=settings.autoAudio;if($('#activityVillageMode'))$('#activityVillageMode').checked=settings.activityVillageMode!==false;if($('#activityVillageMode'))$('#activityVillageMode').checked=settings.activityVillageMode!==false}renderLearningBalanceSettings();updateHome();toast('Cloud progress restored')},
 stats:()=>{const mastered=Object.values(progress).filter(mastery).length,reviews=Number(meta.totalAnswers||0),correct=Number(meta.totalCorrect||0),monsters=Number(meta.totalMonsterVictories||meta.monsterVictories?.length||0),streak=Number(meta.streak||0);unlockStreakAvatars();return{xp:Math.max(0,correct*10+mastered*50+monsters*100+streak*20),mastered,accuracy:reviews?Math.round(correct/reviews*100):0,reviews,monsters_defeated:monsters,streak,avatarUnlocks:[...meta.avatarUnlocks]}},
 experimentalUx:()=>settings.experimentalJourneyUx===true,
 milestoneEmailEvents:()=>{const events=[],cycle=Math.max(0,Number(meta.emailMilestoneCycle||0)),mastered=Object.values(progress).filter(mastery).length;if(chapterStats(0).complete)events.push({programKey:'milestone_first_lesson',eventKey:'first-lesson',cycle});if(mastered>=10)events.push({programKey:'milestone_ten_mastered',eventKey:'ten-mastered',cycle});for(let chapter=0;chapter<wordChapterCount();chapter++)if(chapterStats(chapter).complete)events.push({programKey:'milestone_chapter_complete',eventKey:`chapter-${chapter+1}`,cycle,chapter:chapter+1,title:WORD_CHAPTER_NAMES[chapter]||`Lesson ${chapter+1}`});[3,7,14,30,60].filter(days=>Number(meta.streak||0)>=days).forEach(days=>events.push({programKey:'milestone_streak',eventKey:`streak-${days}`,cycle,days}));return events},
...
const PATH_MILESTONES=[
 ...
 {id:'conversation',icon:'💬',title:'Conversation Town',activity:'Conversation Quest',japanese:'会話クエスト',description:'Choose natural replies while Kai, Aiko and Master speak with you.',requirement:'Start 12 words and answer 5 listening checks correctly.'},
 ...
 {id:'theatre',icon:'🎬',title:'Kaishi Theatre',activity:'Animated Listening Scenes',japanese:'会話劇場',description:'Watch Kai, Aiko and Master act out short Japanese scenes, then prove what you understood.',requirement:'Complete one Conversation Quest.'},
...
function startFirstLesson(){const words=chapterWords(currentWordChapterIndex()).slice(0,3);if(words.length<2){toast('Learning content is still loading');return false}activeFirstLesson=true;activeQuickStep=false;activityReturnScreen='home';pictureGameActive=false;introGuidanceCount=0;session=[...words.flatMap(v=>[{v,skill:'firstEncounter'},{v,skill:'intro'},{v,skill:'pronunciation'}]),...words.map(v=>({v,skill:'meaning'}))];index=0;current=null;clearMissionResume();meta.firstLessonStartedAt=Date.now();save();show('study');renderCurrent();return true}
...
// Keep the headline lesson strength stable while practice expands into new
// dimensions. Optional skills can start at a low score without making a
// successful lesson look weaker; the core progression is meaning, listening
// and reading, which are also the skills used by durable mastery.
function lessonStrengthForWord(word){const skills=progress[word?.id]?.skills||{},core=['meaning','listening','reading'];return Math.round(core.reduce((sum,skill)=>sum+Number(skills[skill]?.strength||0),0)/core.length*100)}
function lessonMasteryChallengePassed(itemIndex){try{const store=loadJSON(profileStorageKey('kq-mastery-challenges-v1'),{});return Boolean(store?.[String(itemIndex)]?.passedAt)}catch{return false}}
function lessonMasterySnapshot(itemIndex){const words=chapterWords(itemIndex),introduced=words.filter(wordIntroduced),strength=words.length?Math.round(words.reduce((sum,word)=>sum+lessonStrengthForWord(word),0)/words.length):0,attempts=words.reduce((sum,word)=>sum+wordPracticeCount(progress[word.id]),0),allIntroduced=words.length>0&&introduced.length===words.length,complete=allIntroduced&&!isAdminTestMode(),allUsable=allIntroduced&&words.every(word=>window.KaishiLearning?.wordState?.(word)==='Usable'),mastered=allUsable&&lessonMasteryChallengePassed(itemIndex);const nextSteps={};words.filter(wordIntroduced).forEach(word=>{const step=masteryStepFor(word);if(!['mastered','retain'].includes(step.id))nextSteps[step.id]=(nextSteps[step.id]||{step,count:0}),nextSteps[step.id].count++});const next=Object.values(nextSteps).sort((a,b)=>b.count-a.count)[0]?.step||null;const stage=!allIntroduced?'not-started':mastered?'mastered':allUsable?'confident':attempts?'strengthening':'learned';const labels={"not-started":'Ready to meet',learned:'Lesson complete',strengthening:'Strengthening',confident:'Confident',mastered:'Durable mastery'};const recommendation=mastered?'Durable mastery comes from returning after a recall gap. Keep these words fresh through normal review.':allUsable?'The lesson is complete. A delayed Mastery Challenge will confirm lasting recall.':next?`The lesson is complete. Next, practise ${next.label.toLowerCase()} for ${nextSteps[next.id].count} word${nextSteps[next.id].count===1?'':'s'}.`:'Meet every word to complete this lesson; confidence grows through later practice.';return{words,introduced:introduced.length,complete,strength,attempts,allUsable,mastered,stage,label:labels[stage],next,recommendation,wordDetails:words.map(word=>({word,strength:lessonStrengthForWord(word),introduced:wordIntroduced(word),next:wordIntroduced(word)?masteryStepFor(word):{label:'Meet this word',reason:'introduce it in the lesson'}}))}}
function makeLessonPracticeSession(itemIndex){const snapshot=lessonMasterySnapshot(itemIndex),words=snapshot.words.filter(wordIntroduced);if(!words.length){toast('Meet this lesson’s words before practising them.');return false}const rounds=[0,1].flatMap(()=>words.map(word=>{const step=masteryStepFor(word),skill=['learn','mastered','retain'].includes(step.id)?chooseSkill(word):step.id;return{v:word,skill}}));session=rounds.slice(0,MISSION_CARD_LIMIT);index=0;current=null;activeVocabularyChapter=itemIndex;clearMissionResume();showJourneySessionPreview(`Lesson ${itemIndex+1} · Focused practice`);return true}
function senseiPathHelpText(snapshot){return`${snapshot.recommendation} Completing a lesson opens the next one; it never requires perfect recall. Focused practice strengthens recognition, listening and recall. Durable mastery is earned when you retrieve the words again after a recall gap.`}
function openSenseiPathHelp(snapshot){const dialog=$('#senseiPathHelpDialog'),text=$('#senseiPathHelpText');if(!dialog||!text)return;text.textContent=senseiPathHelpText(snapshot);if(!dialog.open)dialog.showModal()}
function lessonMasteryPanelHTML(itemIndex){const snapshot=lessonMasterySnapshot(itemIndex),stages=[['learned','Lesson complete'],['strengthening','Strengthening'],['confident','Confident'],['mastered','Durable mastery']],activeIndex=Math.max(0,stages.findIndex(([id])=>id===snapshot.stage));return`<aside class="lesson-mastery-path" style="--lesson-strength:${snapshot.strength}%" aria-label="Sensei’s path: ${esc(snapshot.label)}. Current strength ${snapshot.strength}%."><div class="lesson-mastery-heading"><strong>Sensei’s Path: ${esc(snapshot.label)} <em>(${snapshot.strength}%)</em></strong><button type="button" class="lesson-mastery-help" aria-label="How Sensei’s Path works" title="How Sensei’s Path works">?</button></div><div class="lesson-mastery-stages">${stages.map(([id,label],stageIndex)=>`<span class="${stageIndex<=activeIndex?'reached':''} ${stageIndex===activeIndex?'current':''}"><i>${stageIndex<activeIndex?'✓':stageIndex+1}</i>${esc(label)}</span>`).join('')}</div></aside>`}
...
function chapterStats(itemIndex){const snapshot=lessonMasterySnapshot(itemIndex),reviewed=snapshot.words.filter(item=>wordPracticeCount(progress[item.id])>=2).length;return{...snapshot,reviewed,introTarget:snapshot.words.length,reviewTarget:snapshot.words.length,percent:snapshot.strength}}
window.KaishiLessonMastery={snapshot:lessonMasterySnapshot,startPractice:makeLessonPracticeSession,panel:lessonMasteryPanelHTML};
...
function finishActiveJourneyMission(){
 ...
 const gatable=mission.kind==='activity'&&answers>=4;
...
async function nativeShareInvite(){
 ...
 if(navigator.share){
  try{
  await navigator.share({title:'Kaishi Japanese',text:'Join me on the 1,500-word Japanese journey!',url});
   return;
...
function renderJourney(options={}){
 ...
 renderDailyRoute();
 if(options.renderLegacyHistory!==false)renderHistoryTimeline();
...
function openJourney(section='missions'){
 document.dispatchEvent(new Event('kaishi-journey-render-requested'));
 clearExperimentalPanelOverlay();show('journey');
 try{renderJourney({renderLegacyHistory:!document.body.classList.contains('experimental-journey-enabled')})}catch(error){console.error('Journey route setup failed',error)}
 renderJourneyTimelineWhenReady();requestAnimationFrame(()=>window.scrollTo({top:0,behavior:'auto'}));
 if(section==='current')requestAnimationFrame(()=>document.querySelector('.word-chapter.current')?.scrollIntoView({behavior:'smooth',block:'center'}))
}
...
function kotobaEchoStrengthen(word){const production=pFor(word.id).skills.production;production.attempts++;production.correct++;production.strength=Math.max(0,Math.min(1,Number(production.strength||0)*.75+.75*.25));recordMeaningfulActivity('kotobaEcho');save();window.KaishiCloud?.flush?.()}
function finishKotobaEcho(){const run=kotobaEchoRun;if(!run)return;const firstCount=run.firstRound.length,missed=run.missed.size;meta.kotobaEchoHistory=[...(Array.isArray(meta.kotobaEchoHistory)?meta.kotobaEchoHistory:[]),{at:Date.now(),words:firstCount,correct:run.correct,missed}].slice(-50);save();updateHome();const card=$('#kotobaEchoCard');const rows=run.firstRound.map(word=>{const result=run.results?.[word.id]||{},first=result.first==='correct',retry=result.retry==='correct',strength=Math.round(Number(pFor(word.id).skills.production.strength||0)*100);return '<li><span lang="ja">'+esc(word.word)+'</span><small>'+esc(word.meaning)+' · '+(first?'✓ First try':retry?'✓ Encore':'Needs another try')+' · Production strength '+strength+'%</small></li>'}).join('');card.innerHTML='<section class="kotoba-echo-summary"><span class="kotoba-echo-microphone" aria-hidden="true">🎤</span><span class="eyebrow">Encore complete</span><h2>'+run.correct+' of '+firstCount+' words recognised</h2><p>'+(missed?'You heard the model for every missed word, then gave each one a second chance.':'A clean karaoke run. Your production practice has been recorded.')+'</p><ul class="kotoba-echo-results">'+rows+'</ul><p class="muted">Successful lines strengthen spoken production for that word. Full proficiency still depends on the wider lesson and review skills.</p><button id="kotobaEchoDone" class="primary" type="button">Return to Journey</button></section>';$('#kotobaEchoDone').onclick=()=>{kotobaEchoRun=null;openJourney('missions')}}function kotobaEchoNormalise(value=''){return String(value).replace(/[\s、。！？!?.]/g,'')}
function finishKotobaEchoRecognition(word,transcript,error=''){const run=kotobaEchoRun;if(!run||run.wordId!==word.id)return;clearTimeout(kotobaEchoStopTimer);kotobaEchoStopTimer=0;kotobaEchoRecognition=null;run.listening=false;run.wordId='';const heard=kotobaEchoNormalise(transcript),forms=[word.word,word.reading].map(kotobaEchoNormalise).filter(Boolean),matched=Boolean(heard&&forms.some(form=>heard.includes(form)));if(!heard){renderKotobaEchoFeedback(word,'retry');return}if(matched){run.correct++;run.results=run.results||{};run.results[word.id]={...(run.results[word.id]||{}),[run.phase]:'correct'};kotobaEchoStrengthen(word);renderKotobaEchoFeedback(word,'correct');return}run.results=run.results||{};run.results[word.id]={...(run.results[word.id]||{}),[run.phase]:'wrong'};if(run.phase==='first')run.missed.add(word.id);renderKotobaEchoFeedback(word,'wrong')}
function stopKotobaEchoRecognition(){const run=kotobaEchoRun;if(!run?.listening)return;const button=$('#kotobaEchoStop'),status=$('#kotobaEchoStatus');if(button){button.disabled=true;button.textContent='Checking…'}if(status)status.textContent='Checking your Japanese…';try{kotobaEchoRecognition?.stop()}catch{finishKotobaEchoRecognition(run.word,'','stop-failed')}}
function startKotobaEchoRecognition(word){const Recognition=window.SpeechRecognition||window.webkitSpeechRecognition,run=kotobaEchoRun;if(!Recognition||!run)return;run.listening=true;run.wordId=word.id;run.word=word;run.transcript='';run.error='';const speak=$('#kotobaEchoSpeak'),stop=$('#kotobaEchoStop'),status=$('#kotobaEchoStatus');if(speak)speak.hidden=true;if(stop)stop.hidden=false;if(status)status.textContent='Listening for Japanese… finish naturally or tap Stop.';const recognition=kotobaEchoRecognition=new Recognition();recognition.lang='ja-JP';recognition.interimResults=true;recognition.continuous=false;recognition.onresult=event=>{run.transcript=Array.from(event.results).map(result=>result[0]?.transcript||'').join(' ').trim();if(Array.from(event.results).some(result=>result.isFinal))stopKotobaEchoRecognition()};recognition.onerror=event=>{run.error=event.error||'recognition-error'};recognition.onend=()=>finishKotobaEchoRecognition(word,run.transcript,run.error);try{recognition.start();kotobaEchoStopTimer=setTimeout(stopKotobaEchoRecognition,9000)}catch{finishKotobaEchoRecognition(word,'','start-failed')}}
function renderKotobaEcho(){const run=kotobaEchoRun,card=$('#kotobaEchoCard');if(!run||!card)return;const word=run.queue[run.index];if(!word){if(run.phase==='first'&&run.missed.size){run.phase='retry';run.queue=run.firstRound.filter(item=>run.missed.has(item.id));run.index=0;renderKotobaEcho();return}finishKotobaEcho();return}const roundLabel=run.phase==='retry'?'Encore · missed words':'Main set',progress=`${run.index+1} / ${run.queue.length}`;card.dataset.effect=String((run.index%4)+1);card.innerHTML=`<section class="kotoba-echo-call"><div class="kotoba-echo-lights" aria-hidden="true"><i></i><i></i><i></i></div><div class="kotoba-echo-stage-head"><span>🎤 ${roundLabel}</span><b>${progress}</b></div><div class="kotoba-echo-equalizer" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></div><span class="eyebrow">Your line</span><h2>${esc(word.meaning)}</h2><p>Say the Japanese word naturally. The answer appears after your attempt.</p><div class="kotoba-echo-listening"><button id="kotobaEchoSpeak" class="primary" type="button">🎙️ Check my Japanese</button><button id="kotobaEchoStop" class="kotoba-echo-stop" type="button" hidden>■ Stop listening</button><p id="kotobaEchoStatus" class="kotoba-echo-status" aria-live="polite">Ready when you are.</p></div><p class="kotoba-echo-privacy">Browser recognition checks this attempt. Kaishi Quest does not store your voice or transcript.</p></section>`;$('#kotobaEchoSpeak').onclick=()=>startKotobaEchoRecognition(word);$('#kotobaEchoStop').onclick=stopKotobaEchoRecognition}
function renderKotobaEchoFeedback(word,outcome){const card=$('#kotobaEchoCard'),run=kotobaEchoRun;if(!card||!run)return;const retry=outcome==='retry',correct=outcome==='correct';card.innerHTML=`<section class="kotoba-echo-call kotoba-echo-feedback ${outcome}"><span class="kotoba-echo-microphone" aria-hidden="true">${correct?'✨':'🎤'}</span><span class="eyebrow">${correct?'That landed clearly':retry?'No clear transcript yet':'Hear the model'}</span><h2 lang="ja">${esc(word.word)}</h2><strong>${esc(word.reading)}</strong><p>${correct?'Great delivery. This word now has a little more production strength.':retry?'Move closer to the microphone and try the same word again.':'Listen once, then carry that sound into the next line.'}</p>${retry?'<button id="kotobaEchoRetry" class="primary" type="button">Try this line again</button>':`<button id="kotobaEchoModel" class="audio" type="button">🔊 Hear the Japanese</button><button id="kotobaEchoNext" class="primary" type="button">${run.index===run.queue.length-1?(run.phase==='first'&&run.missed.size?'Start encore →':'See results →'):'Next line →'}</button>`}</section>`;if(retry){$('#kotobaEchoRetry').onclick=()=>renderKotobaEcho();return}$('#kotobaEchoModel').onclick=()=>play(word.wordAudio,word.reading||word.word);if(!correct)play(word.wordAudio,word.reading||word.word);$('#kotobaEchoNext').onclick=()=>{run.index++;renderKotobaEcho()}}
function startKotobaEcho(){activityReturnScreen='journey';const words=kotobaEchoWords();show('kotobaEcho');const card=$('#kotobaEchoCard'),Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(!Recognition){card.innerHTML=`<section class="kotoba-echo-summary"><span class="kotoba-echo-microphone" aria-hidden="true">🎤</span><span class="eyebrow">Speaking support</span><h2>Kotoba Echo needs browser recognition</h2><p>This browser cannot check spoken Japanese here. Try Kotoba Echo in a browser with Japanese speech recognition enabled.</p><button id="kotobaEchoNotReady" type="button">Return to Journey</button></section>`;$('#kotobaEchoNotReady').onclick=()=>openJourney('missions');return}if(words.length<5&&!isAdminTestMode()){card.innerHTML=`<section class="kotoba-echo-summary"><span class="kotoba-echo-microphone" aria-hidden="true">🎤</span><span class="eyebrow">Warm up first</span><h2>Five familiar words unlock the stage</h2><p>Learn ${5-words.length} more familiar word${5-words.length===1?'':'s'}, then return for your first karaoke run.</p><button id="kotobaEchoNotReady" type="button">Return to Journey</button></section>`;$('#kotobaEchoNotReady').onclick=()=>openJourney('missions');return}const firstRound=[...words].sort((left,right)=>Number(pFor(left.id).skills.production.strength||0)-Number(pFor(right.id).skills.production.strength||0)||Math.random()-.5).slice(0,5);kotobaEchoRun={firstRound,queue:firstRound,index:0,phase:'first',missed:new Set(),correct:0,listening:false,wordId:'',word:null,transcript:'',error:'',results:{}};renderKotobaEcho()}
...
function updateHome(){
 ...
 renderSkillScores();updateKanaOverview();updateStreakRescue();renderLearningRhythmWeek();window.KaishiCloud?.renderDashboardAvatar?.();renderJourneyHome();renderOwnerPathControls();window.KaishiBonsai?.render?.()
}
...
function sentenceBridgeWords(words){const currentWords=Array.isArray(words)?words:[];return [...new Map([...vocab.filter(wordIntroduced),...currentWords].map(word=>[word.id,word])).values()]}
function sentenceBridgeEligible(sentence,words){const known=sentenceBridgeWords(words),chunks=sentenceGuideParts(sentence).filter(part=>part.kind==='word'&&part.text);return chunks.every(chunk=>known.some(word=>word.word===chunk.text||word.reading===chunk.text||wordBoundaryMatch(chunk.text,word)))}
function sessionSentencePlan(words){const fresh=words.filter(word=>Number(pFor(word.id).stage||0)===0);if(!fresh.length)return null;const build=(target,companion,sentence)=>{if(!sentenceBridgeEligible(sentence,words))return null;const chunks=sentenceGuideParts(sentence).filter(part=>part.text);if(chunks.length<2)return null;return{target,companion,sentence,meaning:target.sentenceMeaning||target.exampleSentenceMeaning||'',chunks}};for(const target of fresh){const sentence=target.sentence||target.exampleSentence||'';if(!sentence)continue;const companion=fresh.find(word=>word!==target&&wordBoundaryMatch(sentence,word));if(!companion)continue;const plan=build(target,companion,sentence);if(plan)return plan}for(const target of fresh){const sentence=target.sentence||target.exampleSentence||'';if(!sentence)continue;const companion=words.find(word=>word!==target&&Number(pFor(word.id).stage||0)>0&&wordBoundaryMatch(sentence,word));if(!companion)continue;const plan=build(target,companion,sentence);if(plan)return plan}return null}
function connectorState(step){meta.connectorProgress=meta.connectorProgress||{};return meta.connectorProgress[step.id]||(meta.connectorProgress[step.id]={attempts:0,correct:0,stage:0,due:0})}
function dueConnectorStep(currentLesson){const steps=window.KaishiActivityPolicy?.connectorForLesson?Array.from({length:Math.max(0,currentLesson-1)},(_,index)=>window.KaishiActivityPolicy.connectorForLesson(index+1)).filter(Boolean):[];return steps.map(step=>({step,state:connectorState(step)})).filter(item=>item.state.attempts>0&&Number(item.state.due||0)<=Date.now()).sort((a,b)=>Number(a.state.due||0)-Number(b.state.due||0))[0]?.step||null}
function recordConnectorAttempt(step,correct){const state=connectorState(step);state.attempts++;if(correct)state.correct++;state.stage=Math.max(0,Math.min(3,Number(state.stage||0)+(correct?1:0)));const spacing=[1,3,7],days=spacing[Math.min(spacing.length-1,Math.max(0,state.stage-1))];state.due=Date.now()+(correct?days*86400000:10*60*1000);state.updatedAt=Date.now();save(false)}
function renderConnectorCard(item){const step=item.connectorStep,alternatives=(window.KaishiActivityPolicy?.connectorSteps||[]).filter(candidate=>candidate.id!==step.id).slice(0,3).map(candidate=>candidate.explanation);const choices=shuffle([step.explanation,...alternatives]).slice(0,4);const c=$('#card');c.innerHTML=`${senseiBlock(`One small connector makes the words around it work together. Hear it in a useful phrase, then choose its job.`)}<section class="connector-card"><span class="eyebrow">Connector path · ${esc(step.label)}</span><div class="connector-form" lang="ja">${esc(step.form)}</div><h2>${esc(step.label)}</h2><p>${esc(step.explanation)}</p><div class="connector-model"><strong lang="ja">${esc(step.model)}</strong><small>${esc(step.reading)}</small><span>${esc(step.meaning)}</span></div><button id="connectorAudio" class="audio" type="button">🔊 Hear the phrase</button><h3>What does ${esc(step.form)} do here?</h3><div class="choices connector-choices">${choices.map(choice=>`<button class="choice" data-connector-answer="${encodeURIComponent(choice)}">${esc(choice)}</button>`).join('')}</div><p class="connector-note">This is a short grammar step, not a test that can block your Journey.</p></section>`;$('#connectorAudio').onclick=()=>speakJapanese(step.model);document.querySelectorAll('[data-connector-answer]').forEach(button=>button.onclick=()=>{const answer=decodeURIComponent(button.dataset.connectorAnswer),correct=answer===step.explanation;document.querySelectorAll('[data-connector-answer]').forEach(choice=>{choice.disabled=true;if(decodeURIComponent(choice.dataset.connectorAnswer)===step.explanation)choice.classList.add('correct')});button.classList.add(correct?'correct':'wrong');recordConnectorAttempt(step,correct);grade(item.v,'sentence',correct?3:1,correct,false);current.completed=true;saveMissionResume();window.KaishiActivityPolicy?.showAnswerFeedback?.(c,{selected:answer,answer:step.explanation,correct,explanation:step.explanation},next,()=>speakJapanese(step.model))});if(settings.autoAudio)speakJapanese(step.model)}
function bridgeWordHasBeenMet(chunk,plan){if([plan.target,plan.companion].some(word=>sentenceTokenMatchesWord(chunk.text,word)))return true;const matched=vocab.find(word=>sentenceTokenMatchesWord(chunk.text,word)),state=matched&&progress[matched.id];return Boolean(state&&(Number(state.stage||0)>=1||wordPracticeCount(state)>0))}
function sentenceBridgeSupport(plan){const genericMeaning='Meaning clarified by the natural translation',newWords=plan.chunks.filter(chunk=>chunk.kind==='word'&&!bridgeWordHasBeenMet(chunk,plan)&&chunk.meaning&&chunk.meaning!==genericMeaning),connectors=plan.chunks.filter(chunk=>chunk.kind==='particle'||chunk.kind==='grammar'),unique=items=>items.filter((item,index)=>items.findIndex(candidate=>candidate.text===item.text)===index),items=[...unique(newWords),...unique(connectors)];if(!items.length)return'';return`<details class="sentence-bridge-connectors" open><summary>New words and connectors in this sentence</summary><ul>${items.map(chunk=>`<li class="${chunk.kind==='word'?'sentence-bridge-new-word':'sentence-bridge-connector'}"><b lang="ja">${esc(chunk.text)}</b> — ${esc(chunk.meaning||'connects the sentence')}</li>`).join('')}</ul></details>`}
function sentenceBridgeHint(plan,level){if(!level)return "";if(level===1)return `<p class="sentence-bridge-hint-reveal">Start with <b lang="ja">${esc(plan.chunks[0].text)}</b>.</p>`;if(level===2)return `<p class="sentence-bridge-hint-reveal">Then try <b lang="ja">${esc(plan.chunks[1]?.text||plan.chunks[0].text)}</b>.</p>`;return `<p class="sentence-bridge-hint-reveal">The sentence follows the natural order implied by its meaning.</p>`}
function renderSessionSentenceBridge(item){const plan=item.sentencePlan,chosen=[];let hintLevel=0;const freshCompanion=Number(pFor(plan.companion.id).stage||0)===0,connectorList=sentenceBridgeSupport(plan);const draw=()=>{const remaining=plan.chunks.map((chunk,index)=>({chunk,index})).filter(item=>!chosen.includes(item.index));$('#card').innerHTML=`${senseiBlock(freshCompanion?`You met ${plan.target.word} and ${plan.companion.word} together. Build their real sentence from its meaning, then notice how the connecting words hold it together.`:`${plan.target.word} is brand new — ${plan.companion.word} is a word you already know. Build their real sentence together so the new word sticks, and notice how the connecting words hold it together.`)}<div class="eyebrow">Meaning-first sentence bridge</div><h2>${esc(plan.meaning||'Build this complete thought')}</h2><p class="sentence-bridge-hint">Use every Japanese chunk once, in order. Tap a chunk in your sentence to remove it.</p><div class="sentence-bridge-answer" lang="ja">${chosen.length?chosen.map(index=>`<button type="button" class="sentence-bridge-answer-chip ${plan.chunks[index].kind!=='word'?'connector':''}" data-sentence-remove="${index}" lang="ja" aria-label="Remove ${esc(plan.chunks[index].text)} from sentence">${esc(plan.chunks[index].text)}</button>`).join(''):'<span class="muted">Build the Japanese sentence here</span>'}</div><div class="sentence-bridge-choices">${shuffle(remaining).map(({chunk,index})=>`<button type="button" data-sentence-chunk="${index}" lang="ja">${esc(chunk.text)}</button>`).join('')}</div><div class="sentence-bridge-hint-tools"><button type="button" id="sentenceBridgeHint" class="secondary">Need a hint?</button><span class="muted">Hints reduce strength gained.</span></div>${sentenceBridgeHint(plan,hintLevel)}${connectorList}${chosen.length===plan.chunks.length?'<button id="checkSentenceBridge" class="primary reveal">Check my sentence</button>':''}`;document.querySelectorAll('[data-sentence-chunk]').forEach(button=>button.onclick=()=>{chosen.push(+button.dataset.sentenceChunk);draw()});const hint=document.querySelector('#sentenceBridgeHint');if(hint)hint.onclick=()=>{hintLevel=Math.min(3,hintLevel+1);draw()};document.querySelectorAll('[data-sentence-remove]').forEach(button=>button.onclick=()=>{const index=+button.dataset.sentenceRemove;const position=chosen.indexOf(index);if(position>=0)chosen.splice(position,1);draw()});const check=$('#checkSentenceBridge');if(check)check.onclick=()=>{const correct=chosen.every((value,index)=>value===index),answer=$('.sentence-bridge-answer');answer.classList.add(correct?'correct':'wrong');const feedback=document.createElement('p');feedback.className=correct?'ok':'guess-feedback';feedback.textContent=correct?'Excellent — the words work together in a complete Japanese thought.':`The natural order is: ${plan.sentence}`;answer.after(feedback);check.disabled=true;grade(plan.target,'sentence',correct?Math.max(0,3-hintLevel):1,correct,false);current.completed=true;saveMissionResume();window.KaishiActivityPolicy?.showAnswerFeedback?.($('#card'),{selected:chosen.map(i=>plan.chunks[i].text).join(''),answer:plan.sentence,correct,explanation:plan.meaning},next,()=>speakJapanese(plan.sentence))}};draw()}
function renderLessonStoryScene(item){
 const story=item.storyScene;
 if(!story){current.skill='sentence';renderCurrent();return}
 const image=story.image?media(story.image):'';
 $('#card').innerHTML=`${senseiBlock('Aiko and Kai are using today’s Japanese in a real little moment. Answer the scene check first, then review the Japanese and why it works.')}<div class="eyebrow">Aiko & Kai story scene</div><section class="lesson-story-scene"><div class="lesson-story-art">${image?`<img src="${esc(image)}" alt="A mnemonic scene for Aiko and Kai’s sentence">`:''}<div class="lesson-story-cast"><span><img src="media/profiles/girl-base.webp" alt="">Aiko · アイコ</span><span><img src="media/profiles/boy-base.webp" alt="">Kai · カイ</span></div></div><div class="lesson-story-copy" hidden><span class="lesson-story-focus">Notice: ${esc(story.grammarFocus)}</span><p class="lesson-story-sentence" lang="ja">${esc(story.sentence)}</p><small>${esc(story.reading)}</small><p>${esc(story.meaning)}</p><button id="storySentenceAudio" class="audio" type="button">🔊 Hear Aiko and Kai</button></div></section><aside class="lesson-story-note" hidden><b>Why it works</b><p>${esc(story.grammarNote)}</p></aside><section class="lesson-story-check"><span>Quick scene check</span><h2>${esc(story.question.prompt)}</h2><div class="choices">${shuffle(story.question.choices).map(choice=>`<button class="choice" data-answer="${encodeURIComponent(choice)}" type="button">${esc(choice)}</button>`).join('')}</div><div id="storySentenceFeedback" class="teacher-answer-feedback" hidden aria-live="polite"></div></section>`;
 $('#storySentenceAudio').onclick=()=>speakJapanese(story.sentence,null,'kai');
 bindChoices(story.question.answer,'sentence',(ok)=>{const feedback=$('#storySentenceFeedback');if(!feedback)return;const copy=$('.lesson-story-copy'),note=$('.lesson-story-note');if(copy)copy.hidden=false;if(note)note.hidden=false;feedback.hidden=false;feedback.innerHTML=`<p class="${ok?'ok':'guess-feedback'}">${ok?'Exactly. You read the scene correctly.':`Not quite. ${esc(story.question.explanation)}`}</p>${ok?'':`<p lang="ja"><b>${esc(story.sentence)}</b></p><button id="storyAnswerAudio" class="audio" type="button">🔊 Hear the model sentence</button>`}`;const replay=$('#storyAnswerAudio');if(replay)replay.onclick=()=>speakJapanese(story.sentence,null,'kai');if(!ok)speakJapanese(story.sentence,null,'kai')});
}
function makeSession(chapterIndex=null){
 ...
 const queue=selected.map(v=>{const p=pFor(v.id),unknownKana=unknownKanaFor(v).slice(0,1);let skills=p.stage===0?[...(unknownKana.length?['kanaUnlock']:[]),'firstEncounter','intro','pronunciation',...(ankiRecordFor(v)?.sentence?['example']:[]),'meaning']:[chooseSkill(v)];skills=skills.filter(s=>s!=='kanji'||v.word!==v.reading).filter(s=>s!=='picture'||memoryScenes[sceneKey(v)]);return{v,skills}});
 while(queue.some(item=>item.skills.length)&&session.length<MISSION_CARD_LIMIT){const active=queue.filter(item=>item.skills.length).slice(0,ACTIVE_WORD_MIX);active.forEach(item=>{if(item.skills.length&&session.length<MISSION_CARD_LIMIT)session.push({v:item.v,skill:item.skills.shift()})});queue.push(...queue.splice(0,Math.min(ACTIVE_WORD_MIX,queue.length)))}
 ...
 // The core Journey session owns its optional activity, story, and connector
 // cards. Topic sessions reuse this policy separately while their migration is
 // still in progress.
 const lessonIndex=chapterMode?chapterIndex:currentWordChapterIndex(),lessonWords=chapterMode?pool:chapterWords(lessonIndex),schedule=window.KaishiActivitySchedule?.scheduleForLesson?.(lessonIndex,lessonIndex,lessonWords,{forceFirst:true});
 if(schedule?.core?.includes('picture')){
  const pictureCard=window.KaishiActivitySchedule?.buildCard?.('picture',lessonWords);
  if(pictureCard&&!session.some(item=>item.v?.id===pictureCard.v.id&&item.skill==='picture')){
   if(session.length>=MISSION_CARD_LIMIT)session.pop();
   session.splice(Math.max(1,Math.floor(session.length*.65)),0,pictureCard);
  }
 }
 const story=window.KaishiActivityPolicy?.selectLessonStoryScene?.(storySceneCatalog,lessonIndex+1,lessonWords);
 const storyTarget=story?lessonWords.find(word=>word.id===story.targetWordId):null;
 const connector=window.KaishiActivityPolicy?.connectorForLesson?.(lessonIndex+1)||dueConnectorStep(lessonIndex+1);
 if(story&&storyTarget){
  if(session.length>=MISSION_CARD_LIMIT)session.pop();
  session.push({v:storyTarget,skill:'storySentence',storyScene:story});
 }
 if(connector){
  const target=selected[0]||lessonWords[0];
  if(target){
   if(session.length>=MISSION_CARD_LIMIT){
    const removable=session.findIndex(item=>item.skill!=='storySentence');
    session.splice(removable>=0?removable:0,1);
   }
   session.push({v:target,skill:'connector',connectorStep:connector});
  }
 }else if(!story){
  const bridge=sessionSentencePlan(selected);
  if(bridge){
   if(session.length>=MISSION_CARD_LIMIT)session.pop();
   session.push({v:bridge.target,skill:'sessionSentence',sentencePlan:bridge});
  }
 }
...
function showJourneySessionPreview(title='Your Japanese learning session'){
 session=window.KaishiActivityPolicy?.prepareLesson?.(session,unknownKanaFor)||session;
 ...
 if(Number.isInteger(activeVocabularyChapter)){const lesson=window.KaishiActivityPolicy?.buildJourneyCurriculum?.(journeyVocabulary())?.[activeVocabularyChapter];if(lesson)title=`${title.includes('Focused practice')?'Focused practice · ':''}Lesson ${activeVocabularyChapter+1} · ${lesson.arc.title}`;}
...
function makeTargetedMasterySession(ids,skill,title='Your focused practice session'){
 ...
 session=skill==='travel'?words.slice(0,3).flatMap(v=>wordIntroduced(v)?[{v,skill:'meaning'}]:[{v,skill:'kanaUnlock'},{v,skill:'firstEncounter'},{v,skill:'intro'},{v,skill:'meaning'}]):words.map(v=>({v,skill:skill==='retain'?chooseSkill(v):skill}));index=0;current=null;clearMissionResume();showJourneySessionPreview(title);
}
...
function preloadUpcomingCardAudio(count=3){const sources=[];session.slice(index,index+count+1).forEach(item=>{const word=item?.v,record=word&&ankiRecordFor(word);if(word)sources.push(word.wordAudio,word.sentenceAudio,record?.sentenceAudio)});[...new Set(sources.filter(Boolean))].slice(0,8).forEach(preloadAudio)}
...
function exampleSentenceSegments(sentence){try{if(Intl.Segmenter)return[...new Intl.Segmenter('ja',{granularity:'word'}).segment(sentence)].filter(part=>part.segment.trim()).map(part=>({text:part.segment,start:part.index,end:part.index+part.segment.length}))}catch{}return Array.from(sentence).map((text,start)=>({text,start,end:start+text.length}))}
function exampleSentenceMarkup(sentence,word){const targetStart=sentence.indexOf(word),targetEnd=targetStart+word.length;return exampleSentenceSegments(sentence).map(part=>`<span class="example-sentence-word${targetStart>=0&&part.start<targetEnd&&part.end>targetStart?' is-target':''}" data-sentence-start="${part.start}" data-sentence-end="${part.end}">${esc(part.text)}</span>`).join('')}
function highlightExampleSentence(progress){const words=[...document.querySelectorAll('.example-sentence-word')];if(!words.length)return;const length=Math.max(...words.map(word=>Number(word.dataset.sentenceEnd||0)),1),position=Math.max(0,Math.min(length-1,progress*length));let active=words.findIndex(word=>position>=Number(word.dataset.sentenceStart||0)&&position<Number(word.dataset.sentenceEnd||0));if(active<0)active=words.length-1;words.forEach((word,index)=>word.classList.toggle('is-speaking',index===active))}
...
function resolveKanaAnswer(button,entry,answer){
 ...
 state.attempts++;meta.kanaAnswers=Number(meta.kanaAnswers||0)+1;if(ok){state.correct++;meta.kanaCorrect=Number(meta.kanaCorrect||0)+1;state.stage=Math.min(5,state.stage+1)}else state.stage=Math.max(1,state.stage-1);state.updatedAt=Date.now();recordMeaningfulActivity('kana');
...
function openMangaLibrary(){if(!deferredContentReady){show('manga');$('#mangaLibrary').hidden=false;$('#mangaReader').hidden=true;$('#mangaStoryGrid').innerHTML='<p class="muted">Loading the reading library…</p>';void ensureDeferredContent().then(()=>openMangaLibrary());return}mangaStory=null;$('#mangaLibrary').hidden=false;$('#mangaReader').hidden=true;renderMangaLibrary();show('manga')}
...
Object.assign(SENTENCE_GLOSSES,{本当にいい:'really good',奴だ:'a guy (casual)'});
...
const MANGA_NEAR_MISS_JA=[['カイ',['アイコ','マスター']],['アイコ',['カイ','マスター']],['マスター',['カイ','アイコ']],['朝',['夜','午後']],['夜',['朝','昼']],['学校',['家','駅']],['家',['学校','駅']],['元気に',['ゆっくり','静かに']],['小さな',['大きな']],['大きな',['小さな']],['赤い',['青い','白い']],['猫',['犬','鳥']],['子犬',['猫','鳥']],['写真',['手紙','鍵']],['海',['山','川']],['山',['海','町']],['五分',['十分','一時間']],['冷たい',['温かい']]];
const MANGA_NEAR_MISS_EN=[['Kai',['Aiko','Master']],['Aiko',['Kai','Master']],['Master',['Kai','Aiko']],['morning',['evening','afternoon']],['night',['morning','afternoon']],['school',['home','the station']],['full of energy',['slowly','quietly']],['small',['large']],['large',['small']],['red',['blue','white']],['cat',['dog','bird']],['puppy',['cat','bird']],['photograph',['letter','key']],['five minutes',['ten minutes','an hour']],['cold',['warm']],['mountain',['coast','town']]];
...
function openConversationLibrary(){if(!deferredContentReady){show('conversation');$('#conversationGrid').innerHTML='<p class="muted">Loading the conversation library…</p>';void ensureDeferredContent().then(()=>openConversationLibrary());return}conversation=null;$('#conversationLibrary').hidden=false;$('#conversationReader').hidden=true;renderConversationLibrary();show('conversation')}
...
function openTheatreLibrary(){if(!deferredContentReady){show('theatre');$('#theatreGrid').innerHTML='<p class="muted">Loading the theatre library…</p>';void ensureDeferredContent().then(()=>openTheatreLibrary());return}clearTheatreRecording();theatreScene=null;theatreRun=null;clearTheatrePlayback();$('#theatreLibrary').hidden=false;$('#theatreReader').hidden=true;renderTheatreLibrary();show('theatre')}
...
function generatedMnemonic(v){const emoji=sceneEmoji(v),meaning=cleanText(v.meaning),style={clear:'',funny:' Make one detail playfully oversized.',gamer:' Imagine it as a quick game moment.',ghibli:' Give it a warm, hand-drawn feeling.',pokemon:' Imagine a friendly creature causing the moment.'}[settings.mnemonicStyle]||'';return `${emoji} Make one quick mental picture for “${meaning}”, then hear “${v.reading}” inside it.${style}`}
...
function mnemonicVisual(v){return `<div class="memory-stage" aria-hidden="true"><span class="memory-emoji">${sceneEmoji(v)}</span><span class="memory-stage-label">Make a quick mental picture</span></div>`}
function memorySupport(v,force=false){const p=progress[v.id],faded=!force&&p&&Number(p.reps||0)>=4&&Number(p.interval||0)>=7;if(faded)return'<p class="memory-faded">Memory image faded — recall this word directly. Use “Show memory hint” if you need the scaffold again.</p>';const scene=memoryScene(v);return scene||`<section class="memory-cue">${mnemonicVisual(v)}<details class="mnemonic memory-cue-details"${force?' open':''}><summary>Memory cue <span>Optional</span></summary><p>${mnemonic(v)}</p><button id="editMnemonic">Edit cue</button></details></section>`}
...
function renderCurrentUnsafe(){
 ...
 const sessionCounter=$('#sessionCounter');sessionCounter.setAttribute('aria-label',`${game?'Game':'Study'} progress: card ${index+1} of ${session.length}`);sessionCounter.setAttribute('aria-valuemax',String(session.length));sessionCounter.setAttribute('aria-valuenow',String(index+1));sessionCounter.style.setProperty('--session-cards',String(session.length));sessionCounter.innerHTML=session.map((_,cardIndex)=>`<i class="session-progress-chunk ${cardIndex<index?'complete':''} ${cardIndex===index?'current':''}" aria-hidden="true"></i>`).join('');revealed=false;hintUsed=false;startedAt=Date.now();const {v,skill}=current;$('#progressFill').style.width=`${index/session.length*100}%`;const c=$('#card');
if(current.battle){renderBattleQuestion(v);return}
if(current.karuta){renderKarutaQuestion(v);return}
if(skill==='connector'){renderConnectorCard(current);return}
if(skill==='sessionSentence'){renderSessionSentenceBridge(current);return}
if(skill==='storySentence'){renderLessonStoryScene(current);return}
if(skill==='kanaUnlock'){const characters=unknownKanaFor(v).slice(0,2),character=current.character||characters[0],entry=kanaData.find(item=>item.kana===character),topic=topicForWord(v);if(!character){current.skill='firstEncounter';renderCurrent();return}const reading=esc(v.reading||v.word),highlightedReading=reading.replace(esc(character),`<mark>${esc(character)}</mark>`);c.innerHTML=`${senseiBlock(`You need ${character} for ${v.word}. Learn this sound as part of preparing to read ${v.word}.`)}<div class="eyebrow">Character unlock</div><section class="kana-unlock-layout"><div class="kana-unlock-main"><div class="kana-unlock-glyph" lang="ja">${esc(character)}</div><div class="kana-unlock-sound">${esc(entry?.romaji||'New Japanese sound')}</div><button id="kanaUnlockAudio" class="audio primary">🔊 Hear the sound</button></div><aside class="kana-word-preview"><span>Used in this word</span><strong lang="ja">${esc(v.word)}</strong><small>${highlightedReading} · ${esc(v.meaning)}</small><p>This sound appears in the word you are learning.</p></aside></section><button id="kanaUnlockContinue" class="primary reveal">Learn ${esc(character)} now →</button><div class="kana-next-word">Preparing to meet <strong lang="ja">${esc(v.word)}</strong></div>`;$('#kanaUnlockAudio').onclick=()=>entry&&playKana(entry);$('#kanaUnlockContinue').onclick=()=>{markKanaIntroduced(character);next()};if(entry&&settings.autoAudio)playKana(entry);return}
if(skill==='firstEncounter'){const topic=topicForWord(v),reading=v.reading&&v.reading!==v.word?`${esc(v.reading)} · `:'';c.innerHTML=`${senseiBlock('First, simply see and hear this word. We will build the memory next.')}<div class="eyebrow">${esc(topic.icon)} ${esc(topic.title)} · First encounter</div><div class="first-encounter-word" lang="ja">${esc(v.word)}</div><div class="first-encounter-summary">${reading}<strong>${esc(v.meaning)}</strong></div>${writingFormNote(v)}<button id="firstEncounterAudio" class="audio primary">🔊</button><button id="firstEncounterContinue" class="primary reveal">Meet this word →</button>`;$('#firstEncounterAudio').onclick=()=>playWord(v);$('#firstEncounterContinue').onclick=next;if(settings.autoAudio)playWord(v);return}
...
if(skill==='intro'){const topic=topicForWord(v);const hasKanji=kanjiCharacters(v).length>0&&v.word!==v.reading,showFullGuidance=introGuidanceCount<2,reading=v.reading&&v.reading!==v.word?`<span class="reading">${esc(v.reading)}</span><i aria-hidden="true">·</i>`:'';introGuidanceCount++;c.innerHTML=`${senseiBlock('Use the mnemonic image and story to make this word difficult to forget.')}<div class="eyebrow">${esc(topic.icon)} ${esc(topic.title)} · Meet the word</div><section class="meet-word-identity"><div class="meet-word-written"><span>${hasKanji?'Kanji / written word':'Japanese word'}</span><div class="jp">${esc(v.word)}</div><button id="introAudio" class="audio meet-word-speaker" aria-label="Play Japanese word" title="Play Japanese word">🔊</button></div><div class="meet-word-summary">${reading}<strong class="meaning">${esc(v.meaning)}</strong></div></section>${writingFormNote(v)}${visual(v)}${memorySupport(v,true)}<details class="meet-word-guidance"${showFullGuidance?' open':''}><summary>How to learn this word</summary><ol><li>Tap the speaker and say the Japanese word aloud.</li><li>Connect the mnemonic picture and sound clue to the meaning.</li><li>Picture the story, then recall the word once before continuing.</li></ol></details><div class="sticky-study-action"><button id="continueBtn" class="primary reveal">I have linked the word and meaning →</button></div>`;if(settings.autoAudio)playWord(v);$('#introAudio').onclick=()=>playWord(v);$('#continueBtn').onclick=next;wireMemoryEditor(v);return}
if(skill==='pronunciation'){const coachAvailable=Boolean(window.KaishiPronunciation?.available),reading=v.reading&&v.reading!==v.word?`${esc(v.reading)} · `:'';c.innerHTML=`${senseiBlock('Before we test your memory, give this new word one natural attempt. This is optional and will not affect your progress.')}<div class="eyebrow">Optional speaking practice</div><section class="word-pronunciation-card"><div><span>Say this word</span><div class="word-pronunciation-identity"><strong lang="ja">${esc(v.word)}</strong><button id="pronunciationAudio" class="audio" type="button" aria-label="Hear the word" title="Hear the word">🔊</button></div><small>${reading}${esc(v.meaning)}</small></div></section><p class="word-pronunciation-note">In a public place or unable to speak right now? Skip this card with no penalty.</p><div class="word-pronunciation-actions"><button id="pronunciationCheck" class="primary" type="button"${coachAvailable?'':' disabled'}>${coachAvailable?'🎙️ Check my Japanese':'🎙️ Pronunciation check unavailable'}</button><button id="pronunciationSkip" type="button">Skip for now →</button></div><div class="sticky-study-action"><button id="pronunciationContinue" class="primary reveal" type="button">Continue learning →</button></div>`;$('#pronunciationAudio').onclick=()=>play(v.wordAudio);const check=$('#pronunciationCheck');if(check)check.onclick=()=>window.KaishiPronunciation?.open({phrase:v.word,reading:v.reading,meaning:v.meaning,chunks:[v.word],compact:true});$('#pronunciationSkip').onclick=next;$('#pronunciationContinue').onclick=next;if(settings.autoAudio)play(v.wordAudio);return}
...
function recallCard(v,title,front,answer,skill){$('#card').innerHTML=`<div class="eyebrow card-type-badge">Active recall</div><h2 class="recall-question">${title}</h2><div class="jp recall-front">${front}</div><button id="hintBtn" class="hint">Show memory hint</button><div id="answer" hidden><div class="recall-answer-content"><div class="recall-reveal-card"><div class="recall-answer-meaning">${answer}</div></div>${visual(v)}${memorySupport(v)}</div></div><button id="revealBtn" class="primary reveal">Reveal answer</button><div id="ratingsWrap" hidden><p class="rating-title">How easily did you remember this <em>before</em> revealing the answer?</p><div id="ratings" class="ratings"><button class="again" data-rating="1">Again</button><button class="hard" data-rating="2">Hard</button><button class="good" data-rating="3">Good</button><button class="easy" data-rating="4">Easy</button></div></div>`;$('#hintBtn').onclick=()=>showHint(v);$('#revealBtn').onclick=()=>{const answerBox=$('#answer'),hint=$('#hintBtn'),reveal=$('#revealBtn');answerBox.hidden=false;$('#ratingsWrap').hidden=false;reveal.hidden=true;if(hint)hint.remove();const audio=document.createElement('button');audio.id='answerAudio';audio.className='audio recall-inline-audio';audio.type='button';audio.textContent='🔊';audio.title='Play Japanese word';audio.setAttribute('aria-label','Play Japanese word');audio.onclick=()=>playWord(v);const answerKanji=answerBox.querySelector('.jp'),frontKanji=$('#card .recall-front'),target=answerKanji||frontKanji,summary=answerBox.querySelector('.recall-reveal-card');if(target){const row=target.closest('.notebook-word-row')||document.createElement('div');if(!row.parentNode){row.className='recall-word-row';target.parentNode.insertBefore(row,target);row.appendChild(target)}row.appendChild(audio);summary?.prepend(row)}else summary?.prepend(audio);const reading=answerBox.querySelector('.reading');if(reading&&String(reading.textContent||'').replace(/\s+/g,'')===String(v.word||'').replace(/\s+/g,''))reading.remove();wireMemoryEditor(v);wireSceneImages(answerBox)};document.querySelectorAll('[data-rating]').forEach(b=>b.onclick=()=>grade(v,skill,+b.dataset.rating,+b.dataset.rating>=3))}
...
function bindChoices(answer,skill,onReveal){
 const isGuessing=addGuessFlag(),word=current.v;
 document.querySelectorAll('.choice').forEach(button=>button.onclick=()=>{
  if(revealed)return;revealed=true;
  const selected=decodeURIComponent(button.dataset.answer),correct=selected===answer,guessed=isGuessing(),countedCorrect=correct&&!guessed;
  document.querySelectorAll('.choice').forEach(choice=>{choice.disabled=true;if(decodeURIComponent(choice.dataset.answer)===answer)choice.classList.add('correct')});button.classList.add(correct?'correct':'wrong');$('#guessFlag')?.remove();
  if(onReveal)onReveal(correct,guessed);
  const selectedWord=vocab.find(candidate=>[candidate.word,candidate.reading,candidate.meaning].includes(selected));
  grade(word,skill,countedCorrect?(hintUsed?2:3):1,countedCorrect,false);current.completed=true;saveMissionResume();
  const explanation=current.storyScene?.question?.explanation;
  const host=$('#card');host.querySelectorAll('.teacher-answer-feedback').forEach(panel=>panel.remove());
  const storySentence=current.storyScene?.sentence;
  if(window.KaishiActivityPolicy?.showAnswerFeedback)window.KaishiActivityPolicy.showAnswerFeedback(host,{selected,answer,correct,guessed,word:skill==='sentence'?undefined:word,selectedWord,explanation},next,()=>storySentence?speakJapanese(storySentence,null,'kai'):skill==='sentence'?playSentence(word):playWord(word));
  else {const proceed=document.createElement('button');proceed.textContent='Continue';proceed.className='primary';proceed.onclick=next;host.append(proceed)}
 });
}
...
function finishSession(){clearMissionResume();recordSessionHistory();const quickCompleted=activeQuickStep,firstLessonCompleted=activeFirstLesson;activeQuickStep=false;activeFirstLesson=false;const completedJourneyMission=Boolean(activeJourneyMission)&&finishActiveJourneyMission();if(battleActive){showBattleSummary();return}if(karutaActive){showKarutaSummary();return}if(pictureGameActive){abortSession('games');toast('Game complete 🎉');return}save();updateHome();if(completedJourneyMission)renderJourney();const c=$('#card');$('#sessionCounter').textContent='Complete';$('#progressFill').style.width='100%';if(firstLessonCompleted){c.innerHTML=`${senseiBlock('You practised recalling your first Japanese words. That is a real beginning.',true,'celebrating')}<div class="eyebrow">First lesson complete</div><h2>Your first learning stamp is earned.</h2><p>You have started practising three useful words. Keep building a rhythm through small, meaningful study days.</p><div class="mission-complete-actions"><button id="finishMissionNow" class="primary">See my progress</button><button id="keepLearningMission">Learn a little more</button></div>`;$('#finishMissionNow').onclick=()=>{show('home');setTimeout(()=>$('#learningRhythmWeek')?.scrollIntoView({behavior:'smooth',block:'center'}),60)};$('#keepLearningMission').onclick=()=>{session=[];index=0;current=null;startTopicSession(currentTopic().id)};return}if(quickCompleted){c.innerHTML=`${senseiBlock('Quick Step complete. Even a short, focused practice keeps your learning rhythm moving.',true,'celebrating')}<div class="eyebrow">Rhythm protected</div><h2>A small step still counts.</h2><p>Three genuine review attempts protected today without turning practice into a chore.</p><div class="mission-complete-actions"><button id="finishMissionNow" class="primary">Back to dashboard</button><button id="keepLearningMission">Keep learning</button></div>`;$('#finishMissionNow').onclick=()=>show('home');$('#keepLearningMission').onclick=()=>{session=[];index=0;current=null;startTopicSession(currentTopic().id)};return}c.innerHTML=`${senseiBlock('Session complete. Your progress is saved.')}<div class="eyebrow">Mission complete</div><h2>Great work — you reached today’s save point.</h2><p>You can finish now, or continue with another short optional mission.</p><div class="mission-complete-actions"><button id="finishMissionNow" class="primary">Finish for now</button><button id="keepLearningMission">Keep learning</button></div>`;$('#finishMissionNow').onclick=()=>{activityReturnScreen='journey';renderJourney();show('journey');toast(todayActivity().qualified?'Mission complete — today’s rhythm is protected 🎉':'Mission complete — progress saved')};$('#keepLearningMission').onclick=()=>{session=[];index=0;current=null;startTopicSession(currentTopic().id)}}
...
function showUpdateBanner(info){latestVersionInfo=info;$('#updateTitle').textContent=`Kaishi Japanese ${info.version} is available`;$('#updateSummary').textContent=(info.changes||[]).slice(0,2).join(' • ')||'A new version is ready.';$('#updateBanner').hidden=false;$('#updateStatus').textContent=`Update available: v${info.version}`}
async function checkForUpdates(manual=false){
 $('#updateBanner').hidden=true;
 $('#updateStatus').textContent=`You’re running Kaishi Japanese v${APP_VERSION}.`;
...
async function ensureDeferredContent(){
 if(deferredContentReady)return;
 if(deferredContentPromise)return deferredContentPromise;
 deferredContentPromise=Promise.all([
 ...
 ]).then(([manga,conversationData,theatre,grammar,components])=>{
  mangaStories=manga;conversations=conversationData;theatreScenes=theatre;grammarLessons=grammar;componentData=components;deferredContentReady=true;updateHome();
 });
 return deferredContentPromise;
}
async function init(){
 $('#updateBanner').hidden=true;
 $('#card').innerHTML='<div class="eyebrow">Loading</div><h2>Preparing Kaishi Japanese…</h2>';
 try{
  [vocab,kanaData,memoryScenes,ankiContent,topicData,learningGraph,katakanaCore,vocabularyOrthography,storySceneCatalog]=await Promise.all([
   ...
   ,safeFetchJson(`data/lesson-story-scenes.json?v=${APP_VERSION}`,{schemaVersion:1,scenes:[]})
  ]);
  ...
  updateHome();
  appReady=true;
  const defer=window.requestIdleCallback||((callback)=>window.setTimeout(callback,1200));
  defer(()=>{void ensureDeferredContent()});
  ...
  $('#card').innerHTML=`<div class="eyebrow">Loading error</div><h2>Kaishi Japanese could not start.</h2><p class="muted">${esc(e?.message||'Unknown error')}</p>`;
 }
...
$('#kotobaEchoBack').onclick=()=>{clearTimeout(kotobaEchoStopTimer);try{kotobaEchoRecognition?.abort()}catch{}kotobaEchoRecognition=null;kotobaEchoRun=null;openJourney('missions')};
...
window.KaishiActivityPolicy={...(window.KaishiActivityPolicy||{}),deviceRepair:{
 ...
 workshop:()=>{const base={schemaVersion:1,xp:0,level:1,selectedSkin:'bench',unlockedSkins:['bench'],unlockedModules:['standard'],soundPack:'mechanical',updatedAt:0},workshop={...base,...(meta.deviceRepairProgress?.workshop||{})};workshop.unlockedSkins=Array.isArray(workshop.unlockedSkins)?workshop.unlockedSkins:['bench'];workshop.unlockedModules=Array.isArray(workshop.unlockedModules)?workshop.unlockedModules:['standard'];return workshop;},
 finishRun:(run,success,score)=>{meta.deviceRepairProgress=meta.deviceRepairProgress||{schemaVersion:2,devices:{}};const devices=meta.deviceRepairProgress.devices||(meta.deviceRepairProgress.devices={}),state=devices[run.device]||(devices[run.device]={repairs:0,bestScore:0});if(success){state.repairs=Number(state.repairs||0)+1;state.bestScore=Math.max(Number(state.bestScore||0),Number(score||0));state.updatedAt=Date.now()}const workshop=window.KaishiActivityPolicy.deviceRepair.workshop(),awarded=Math.max(0,Math.round((success?100:25)+Math.min(500,Math.max(0,Number(score)||0))/5));workshop.xp+=awarded;workshop.level=Math.min(10,Math.floor(workshop.xp/500)+1);if(workshop.level>=2&&!workshop.unlockedSkins.includes('midnight'))workshop.unlockedSkins.push('midnight');if(workshop.level>=4&&!workshop.unlockedSkins.includes('sakura'))workshop.unlockedSkins.push('sakura');if(workshop.level>=3&&!workshop.unlockedModules.includes('advanced'))workshop.unlockedModules.push('advanced');workshop.updatedAt=Date.now();meta.deviceRepairProgress.workshop=workshop;delete meta.deviceRepairProgress.activeRun;save();updateHome();},
},kotobaCheckpoint:{
 show:(id)=>show(id),
