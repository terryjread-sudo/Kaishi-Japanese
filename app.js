...
function lessonMasteryPanelHTML(itemIndex){const snapshot=lessonMasterySnapshot(itemIndex),stages=[['learned','Learned'],['strengthening','Strengthening'],['confident','Confident'],['mastered','Mastered']],activeIndex=Math.max(0,stages.findIndex(([id])=>id===snapshot.stage));return`<aside class="lesson-mastery-path" style="--lesson-strength:${snapshot.strength}%" aria-label="Sensei’s path: ${esc(snapshot.label)}. Current strength ${snapshot.strength}%."><div class="lesson-mastery-heading"><strong>Sensei’s Path: ${esc(snapshot.label)} <em>(${snapshot.strength}%)</em></strong></div><div class="lesson-mastery-stages">${stages.map(([id,label],stageIndex)=>`<span class="${stageIndex<=activeIndex?'reached':''} ${stageIndex===activeIndex?'current':''}"><i>${stageIndex<activeIndex?'✓':stageIndex+1}</i>${esc(label)}</span>`).join('')}</div><details><summary>How this works</summary><p>${esc(snapshot.recommendation)} Meeting every word opens the next lesson. Focused practice builds confidence through recognition, listening and recall. Mastery is earned later, when you can retrieve the words again after a recall gap.</p></details></aside>`}
...
function renderCurrent(){
 try{
  ...
  if(current?.v&&!current.battle&&!current.karuta)addNotebookSaveControl(current.v);
   if(current?.v&&!pictureGameActive&&!karutaActive&&!battleActive){const chapter=Number.isInteger(activeVocabularyChapter)?activeVocabularyChapter:Math.floor(vocab.indexOf(current.v)/WORD_CHAPTER_SIZE),panel=document.createElement('div');panel.innerHTML=lessonMasteryPanelHTML(Math.max(0,chapter));$('#card')?.prepend(panel.firstElementChild);const step=masteryStepFor(current.v),banner=document.createElement('aside');banner.className='word-mastery-next';banner.innerHTML=`<span>Journey</span><b>Next: ${esc(step.label)}</b><small>${esc(step.reason)}</small>`;$('#card')?.prepend(banner)}
