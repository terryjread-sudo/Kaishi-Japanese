...
function pathUnlocked(id){return isAdminTestMode()||(id==='theatre'&&theatreScenes.some(scene=>scene.featured))||meta.activityPurchases.includes(id)||meta.pathOverrides.includes(id)}
...
function mangaComicWords(words=[]){return words.map(word=>word.reading||word.meaning?`<button class="manga-comic-word" lang="ja" data-comic-speech="${encodeURIComponent(word.speech||word.text)}" aria-label="Hear ${esc(word.reading||word.text)}" title="${esc(`${word.reading||word.text}${word.meaning?` · ${word.meaning}`:''}`)}">${esc(word.text)}</button>`:`<span lang="ja">${esc(word.text)}</span>`).join('')}
function mangaComicBalloon(balloon,index){return `<aside class="manga-comic-balloon ${esc(balloon.tone||'speech')}" style="--balloon-x:${Number(balloon.x)||5}%;--balloon-y:${Number(balloon.y)||5}%;--balloon-w:${Number(balloon.width)||34}%" aria-label="Dialogue ${index+1}"><div class="manga-comic-japanese">${mangaComicWords(balloon.words)}</div><p class="manga-comic-english">${esc(balloon.english)}</p><small>${esc(balloon.speaker||'')}</small></aside>`}
function renderMangaComicPage(){
 const page=mangaStory.pages[mangaPanelIndex],last=mangaPanelIndex===mangaStory.pages.length-1;$('#mangaPanelCounter').textContent=`Page ${mangaPanelIndex+1}/${mangaStory.pages.length} · right to left`;
 $('#mangaReaderContent').innerHTML=`<section class="manga-comic-reader ${mangaComicEnglish?'show-english':''}" tabindex="0" aria-label="Right-to-left manga page ${mangaPanelIndex+1} of ${mangaStory.pages.length}"><header class="manga-comic-header"><div><span class="eyebrow">本格漫画 · Read right → left</span><h2 lang="ja">${esc(mangaStory.title)}</h2><p>${esc(mangaStory.englishTitle)}</p></div><button id="mangaComicLanguage" class="manga-language-toggle" aria-pressed="${mangaComicEnglish}">${mangaComicEnglish?'日本語を見る':'English text'}</button></header><div class="manga-comic-page"><img src="${esc(page.image)}?v=${APP_VERSION}" alt="${esc(page.alt)}">${page.balloons.map(mangaComicBalloon).join('')}<span class="manga-read-direction" aria-hidden="true">START →</span></div><p class="manga-comic-help">Tap any Japanese word to hear it · swipe left for the next page</p><nav class="manga-comic-pagination" aria-label="Manga pages"><button id="mangaComicPrevious"${mangaPanelIndex?'':' disabled'}>→ Previous</button><div class="manga-comic-dots" dir="rtl">${mangaStory.pages.map((_,index)=>`<button data-comic-page="${index}" class="${index===mangaPanelIndex?'active':''}" aria-label="Page ${index+1}"${index===mangaPanelIndex?' aria-current="page"':''}></button>`).join('')}</div><button id="mangaComicNext" class="primary">${last?'Finish story':'Next page ←'}</button></nav></section>`;
 document.querySelectorAll('[data-comic-speech]').forEach(button=>button.onclick=event=>{event.stopPropagation();speakJapanese(decodeURIComponent(button.dataset.comicSpeech))});$('#mangaComicLanguage').onclick=()=>{mangaComicEnglish=!mangaComicEnglish;renderMangaComicPage()};$('#mangaComicPrevious').onclick=()=>changeMangaComicPage(-1);$('#mangaComicNext').onclick=()=>last?finishMangaComic():changeMangaComicPage(1);document.querySelectorAll('[data-comic-page]').forEach(button=>button.onclick=()=>{mangaPanelIndex=+button.dataset.comicPage;renderMangaComicPage()});const reader=document.querySelector('.manga-comic-reader');let swipeX=0;reader.onpointerdown=event=>swipeX=event.clientX;reader.onpointerup=event=>{const distance=event.clientX-swipeX;if(Math.abs(distance)>55){if(distance<0&&!last)changeMangaComicPage(1);if(distance>0&&mangaPanelIndex)changeMangaComicPage(-1)}};reader.onkeydown=event=>{if(event.key==='ArrowLeft'&&!last)changeMangaComicPage(1);if(event.key==='ArrowRight'&&mangaPanelIndex)changeMangaComicPage(-1)};
}
function changeMangaComicPage(direction){mangaPanelIndex=Math.max(0,Math.min(mangaStory.pages.length-1,mangaPanelIndex+direction));const state=mangaProgressFor(mangaStory.id);state.lastPanel=Math.max(state.lastPanel,mangaPanelIndex);state.updatedAt=Date.now();save(false);renderMangaComicPage()}
...
async function init(){
 ...
 const versionCard=$('.version-card');if(versionCard){versionCard.querySelector('strong').textContent=`Kaishi Quest v${APP_VERSION}`;versionCard.querySelector('span').textContent='Five-Act Theatre';versionCard.querySelector('small').textContent='Watch, understand, explore, join and perform practical Japanese in animated real-world episodes.'}
