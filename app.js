...
function renderLearningNotebook(tab='words'){
 ...
 content.innerHTML=`<section class="notebook-paper"><div class="notebook-paper-heading"><div><span>${items.length} saved</span><h3>${heading}</h3></div><button id="notebookClearTab" type="button"${items.length?'':' disabled'}>Clear ${isWords?'words':'sentences'}</button></div>${items.length?`<div class="notebook-entry-list">${entries}</div>`:`<div class="notebook-empty"><span>📓</span><p>${empty}</p></div>`}<button id="notebookResetAll" class="notebook-reset" type="button"${words.length||sentences.length?'':' disabled'}>Reset notebook</button></section>`;
...
$('#dashboardNotebook').onclick=()=>openLearningNotebook('words');
