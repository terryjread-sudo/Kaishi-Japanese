...
function addNotebookSaveControl(word){const card=$('#card');if(!card||!word||card.querySelector('[data-notebook-save-word]'))return;const saved=notebookWordSaved(word),button=document.createElement('button');button.type='button';button.className=`notebook-save-control${saved?' saved':''}`;button.dataset.notebookSaveWord=word.id;button.textContent=saved?'★ In notebook':'☆ Save word';button.onclick=()=>{const isSaved=toggleNotebookWord(word);button.classList.toggle('saved',isSaved);button.textContent=isSaved?'★ In notebook':'☆ Save word'};card.prepend(button)}
...
function lessonMasteryPanelHTML(itemIndex){const snapshot=lessonMasterySnapshot(itemIndex),stages=[['learned','Learned'],['strengthening','Strengthening'],['confident','Confident'],['mastered','Mastered']],activeIndex=Math.max(0,stages.findIndex(([id])=>id===snapshot.stage));return`<aside class="lesson-mastery-path" aria-label="Sensei’s lesson path: ${esc(snapshot.label)}. Current strength ${snapshot.strength}%."><div class="lesson-mastery-heading"><span>Sensei’s lesson path</span><strong>${esc(snapshot.label)}</strong></div><div class="lesson-mastery-meter" role="progressbar" aria-valuemin="0" aria-valuemax="100" aria-valuenow="${snapshot.strength}" aria-label="Current recall strength ${snapshot.strength}%"><i style="width:${snapshot.strength}%"></i></div><div class="lesson-mastery-stages">${stages.map(([id,label],stageIndex)=>`<span class="${stageIndex<=activeIndex?'reached':''} ${stageIndex===activeIndex?'current':''}"><i>${stageIndex<activeIndex?'✓':stageIndex+1}</i>${esc(label)}</span>`).join('')}</div><p><b>Current strength: ${snapshot.strength}%</b> · ${esc(snapshot.recommendation)}</p><details><summary>How lesson strength works</summary><p>Meeting every word opens the next lesson. Focused practice builds confidence through recognition, listening and recall. Mastery is earned later, when you can retrieve the words again after a recall gap.</p></details></aside>`}
