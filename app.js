...
function renderCurrent(){
 ...
 try{
  ...
  if(current?.v&&!pictureGameActive&&!karutaActive&&!battleActive){const chapter=Number.isInteger(activeVocabularyChapter)?activeVocabularyChapter:Math.floor(journeyVocabulary().indexOf(current.v)/WORD_CHAPTER_SIZE),snapshot=lessonMasterySnapshot(Math.max(0,chapter)),panel=document.createElement('div');panel.innerHTML=lessonMasteryPanelHTML(Math.max(0,chapter));const masteryPath=panel.firstElementChild;$('#card')?.prepend(masteryPath);masteryPath?.querySelector('.lesson-mastery-help')?.addEventListener('click',()=>openSenseiPathHelp(snapshot));const step=masteryStepFor(current.v),banner=document.createElement('aside');banner.className='word-mastery-next';banner.innerHTML=`<span>Journey</span><b>Next: ${esc(step.label)}</b><small>${esc(step.reason)}</small>`;$('#card')?.prepend(banner)}
...
function next(){if(activeQuickStep&&Math.max(0,todayActivity().tested-Number(todayActivity().quickStartTested||0))>=3){session=session.slice(0,3);index=session.length;finishSession();renderCurrent();return}saveMissionResume();const completed=index+1;if(completed<session.length&&completed%CHECKPOINT_INTERVAL===0&&!pictureGameActive&&!karutaActive&&!battleActive){window.KaishiCloud?.flush?.();toast('Saving progress…')}index++;renderCurrent()}
