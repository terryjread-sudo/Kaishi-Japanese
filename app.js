...
function captureJourneyScrollState(){
 const state={window:0,journey:0,track:0,history:0,active:document.activeElement};
 try{state.window=Number(window.scrollY||document.documentElement.scrollTop||document.body.scrollTop||0)}catch{}
 try{state.journey=Number($('#journey')?.scrollTop||0)}catch{}
 try{state.track=Number($('#journeyHistoryTrack')?.scrollTop||0);if($('#journeyHistoryTrack'))$('#journeyHistoryTrack').dataset.kqJourneySavedScroll=String(state.track)}catch{}
 try{state.history=Number($('#journeyHistoryTimeline')?.scrollTop||0)}catch{}
 return state;
}
function restoreJourneyScrollState(state){
 if(!state)return;
 const apply=()=>{
  try{if(document.querySelector('#journey:not([aria-hidden="true"])'))window.scrollTo({top:state.window,left:0,behavior:'auto'})}catch{try{window.scrollTo(0,state.window)}catch{}}
  try{$('#journey').scrollTop=state.journey}catch{}
  try{$('#journeyHistoryTrack').scrollTop=state.track}catch{}
  try{$('#journeyHistoryTimeline').scrollTop=state.history}catch{}
 };
 requestAnimationFrame(()=>{apply();requestAnimationFrame(apply);setTimeout(apply,80)});
}
let journeyScrollGuardInstalled=false;
function installJourneyScrollGuard(){
 if(journeyScrollGuardInstalled)return;
 journeyScrollGuardInstalled=true;
 const remember=()=>{
  if($('#journey')?.getAttribute('aria-hidden')==='true')return;
  const screen=$('#journey');
  if(screen)screen.dataset.kqJourneyUserScrolled='1';
  try{screen.dataset.kqJourneyWindowScroll=String(window.scrollY||document.documentElement.scrollTop||0)}catch{}
 };
 window.addEventListener('scroll',remember,{passive:true,capture:true});
 document.addEventListener('touchmove',()=>{if($('#journey'))$('#journey').dataset.kqJourneyUserScrolled='1'},{passive:true,capture:true});
 document.addEventListener('wheel',()=>{if($('#journey'))$('#journey').dataset.kqJourneyUserScrolled='1'},{passive:true,capture:true});
 const observer=new MutationObserver(()=>{
  const screen=$('#journey');
  if(!screen||screen.getAttribute('aria-hidden')==='true'||screen.dataset.kqJourneyUserScrolled!=='1')return;
  const y=Number(screen.dataset.kqJourneyWindowScroll||0), track=Number($('#journeyHistoryTrack')?.dataset.kqJourneySavedScroll||0);
  requestAnimationFrame(()=>{
   try{window.scrollTo({top:y,left:0,behavior:'auto'})}catch{try{window.scrollTo(0,y)}catch{}}
   try{if($('#journeyHistoryTrack'))$('#journeyHistoryTrack').scrollTop=track}catch{}
  });
 });
 observer.observe(document.body,{subtree:true,childList:true});
}
function renderJourney(){
 const preserve=captureJourneyScrollState();
 settings.playMode='journey';refreshPathUnlocks();
 const currentIndex=pathCurrentIndex(),unlocked=PATH_MILESTONES.filter(item=>pathUnlocked(item.id)).length,wordPosition=wordJourneyPosition(),masteryState=masterySnapshot();
 $('#journeyStats').innerHTML=`<article><strong>${unlocked}/${PATH_MILESTONES.length}</strong><span>Activities unlocked</span></article><article><strong>${wordPosition.explored}/${vocab.length}</strong><span>Words introduced</span></article><article><strong>${masteryState.mastered}</strong><span>Full mastery journeys</span></article><article><strong>${wordPosition.completed}/${wordPosition.total}</strong><span>Chapters completed</span></article>`;
 renderDailyRoute();
 $('#pathRoad').innerHTML=PATH_MILESTONES.map((item,itemIndex)=>{const unlockedItem=pathUnlocked(item.id),visited=Boolean(meta.pathVisits[item.id]),current=itemIndex===currentIndex;return `<article class="path-stop ${unlockedItem?'unlocked':'locked'} ${visited?'visited':''} ${current?'current':''}"><div class="path-marker">${esc(item.icon)}</div>${current?`<img id="journeyAvatar" class="path-avatar" src="${pathAvatarSource()}" alt="Your Kaishi character at ${esc(item.title)}">`:''}<button data-path-launch="${esc(item.id)}"${unlockedItem?'':' disabled'}><span class="path-status">${unlockedItem?(visited?'✓':'→'):'🔒'}</span><span><b>${esc(item.title)}</b><small>${esc(item.activity)}</small></span></button><p>${esc(item.description)}</p><small class="path-requirement">${esc(pathRequirementText(item.id))}</small></article>`}).join('');
 document.querySelectorAll('[data-path-launch]').forEach(button=>button.onclick=()=>launchPathMilestone(button.dataset.pathLaunch));
 renderOwnerPathControls();
 installJourneyScrollGuard();
 restoreJourneyScrollState(preserve);
}
function openJourney(section='missions'){
 const wasVisible=$('#journey')?.getAttribute('aria-hidden')!=='true';
 const preserve=wasVisible?captureJourneyScrollState():null;
 renderJourney();show('journey');
 if(section==='practice')requestAnimationFrame(()=>$('#practiceHubTitle')?.scrollIntoView({behavior:'smooth',block:'start'}));
 else if(section==='current')requestAnimationFrame(()=>{const hasNew=PATH_MILESTONES.some(item=>pathUnlocked(item.id)&&!meta.pathVisits[item.id]);$(hasNew?'.path-stop.current':'.word-chapter.current')?.scrollIntoView({behavior:'smooth',block:'center'})});
 else if(!wasVisible && !preserve)requestAnimationFrame(()=>$('#dailyRouteTitle')?.scrollIntoView({behavior:'smooth',block:'start'}));
 else if(preserve)restoreJourneyScrollState(preserve);
}
...
function renderOwnerPathControls(owner=window.KaishiCloud?.isOwner?.()){
 const controls=$('#ownerPathControls');if(!controls)return;controls.hidden=!owner;if(!owner)return;
 const current=Math.max(0,Math.min(currentWordChapterIndex(),Math.max(0,wordChapterCount()-1)));
 const lessonGrid=controls.querySelector('#ownerLessonGrid');
 if(lessonGrid){
  lessonGrid.innerHTML=Array.from({length:Math.min(wordChapterCount(),30)},(_,itemIndex)=>{
   const stats=chapterStats(itemIndex),isCurrent=itemIndex===current,unlocked=chapterUnlocked(itemIndex);
   return `<button type="button" data-owner-lesson="${itemIndex}" class="${isCurrent?'current ':''}${stats.complete?'complete ':''}${unlocked?'unlocked':''}"><b>Lesson ${itemIndex+1}</b><span>${esc(WORD_CHAPTER_NAMES[itemIndex]||`Lesson ${itemIndex+1}`)}</span><small>${stats.complete?'Complete':isCurrent?'Current':unlocked?'Available':'Locked'}</small></button>`;
  }).join('');
 }
 const eventGrid=controls.querySelector('#ownerEventGrid');
 if(eventGrid){
  eventGrid.innerHTML=PATH_MILESTONES.map((item,itemIndex)=>{
   const unlocked=pathUnlocked(item.id);
   return `<button type="button" data-owner-event="${esc(item.id)}" class="${unlocked?'unlocked':''}"><span>${esc(item.icon)}</span><b>${esc(item.title)}</b><small>${unlocked?'Unlocked':'Locked'}</small></button>`;
  }).join('');
 }
 const status=controls.querySelector('#ownerJourneyStatus');
 if(status)status.textContent=`Test state: Lesson ${current+1} · ${PATH_MILESTONES.filter(item=>pathUnlocked(item.id)).length}/${PATH_MILESTONES.length} activities unlocked`;
 const test=controls.querySelector('#ownerTestMode');
 if(test)test.checked=isAdminTestMode();
}
function unlockOwnerPathThrough(itemIndex){
 if(!window.KaishiCloud?.isOwner?.())return;
 const ids=PATH_MILESTONES.slice(0,itemIndex+1).map(item=>item.id);
 meta.pathOverrides=[...new Set([...meta.pathOverrides,...ids])];save();renderOwnerPathControls(true);renderJourneyHome();toast(`Journey activities unlocked through ${PATH_MILESTONES[itemIndex].title}`)
}
function unlockOwnerChapterThrough(itemIndex){
 if(!window.KaishiCloud?.isOwner?.())return;
 meta.chapterOverrides=[...new Set([...meta.chapterOverrides,...Array.from({length:itemIndex+1},(_,index)=>index)])];save();renderOwnerPathControls(true);renderJourneyHome();toast(`Journey unlocked through Lesson ${itemIndex+1}`)
}
function setOwnerLesson(itemIndex){
 if(!window.KaishiCloud?.isOwner?.())return;
 const index=Math.max(0,Math.min(Number(itemIndex)||0,wordChapterCount()-1));
 meta.chapterOverrides=[...new Set([...meta.chapterOverrides,...Array.from({length:index+1},(_,i)=>i)])];
 meta.pathOverrides=[...new Set([...meta.pathOverrides,'vocabulary'])];
 const route=meta.dailyJourneyRoute;
 if(route?.steps?.length){
  const step=route.steps.find(item=>item.kind==='chapter'&&!item.retryOf);
  if(step)step.chapter=index;
 }
 save();renderOwnerPathControls(true);renderJourneyHome();renderJourney();toast(`Test learner positioned at Lesson ${index+1}`)
}
function toggleOwnerEvent(id){
 if(!window.KaishiCloud?.isOwner?.())return;
 if(!meta.pathOverrides.includes(id))meta.pathOverrides.push(id);
 save();renderOwnerPathControls(true);renderJourneyHome();toast(`${PATH_MILESTONES.find(item=>item.id===id)?.title||id} unlocked for testing`)
}
function resetOwnerJourneyTest(){
 if(!window.KaishiCloud?.isOwner?.())return;
 meta.pathOverrides=[];meta.chapterOverrides=[];meta.pathVisits={};meta.dailyJourneyRoute=null;
 save();renderOwnerPathControls(true);renderJourneyHome();renderJourney();toast('Journey test state reset')
}
function toggleOwnerTestMode(enabled){
 if(!window.KaishiCloud?.isOwner?.())return;
 try{
  if(enabled)sessionStorage.setItem(ADMIN_TEST_MODE_KEY,'1');else sessionStorage.removeItem(ADMIN_TEST_MODE_KEY);
 }catch{}
 location.reload();
}
