...
let vocab=[], kanaData=[], mangaStories=[], conversations=[], theatreScenes=[], grammarLessons=[], componentData={components:{},kanji:[]}, ankiContent={records:[]}, topicData={topics:[]}, session=[], index=0, current=null, revealed=false, startedAt=0, hintUsed=false, memoryScenes={};
...
meta.chapterOverrides=Array.isArray(meta.chapterOverrides)?[...meta.chapterOverrides]:[];meta.topicProgress=meta.topicProgress||{};
...
function currentTopic(){return journeyTopics()[currentTopicIndex()]||{id:'core-japanese',title:'Core Japanese',icon:'🗾',words:vocab}}
...
function startTopicSession(topicId=currentTopic().id){const topic=journeyTopics().find(item=>item.id===topicId)||currentTopic();activityReturnScreen='journey';const unseen=topic.words.filter(word=>!progress[word.id]||Number(progress[word.id].stage||0)===0).slice(0,settings.newLimit),due=topic.words.filter(word=>progress[word.id]&&Number(progress[word.id].due||0)<=Date.now()).slice(0,settings.sessionSize);const selected=[...due,...unseen];if(!selected.length){makeTargetedMasterySession(topic.words.slice(0,Math.min(settings.sessionSize,topic.words.length)).map(word=>word.id),topicWeakestSkill(topic));return}session=[];selected.forEach(v=>{const p=pFor(v.id),unknownKana=unknownKanaFor(v).slice(0,2),skills=p.stage===0?[...(unknownKana.length?['kanaUnlock']:[]),'firstEncounter','intro',...(ankiRecordFor(v)?.sentence?['example']:[]),'meaning','listening','reading','picture']:[chooseSkill(v)];skills.filter(skill=>skill!=='picture'||memoryScenes[sceneKey(v)]).forEach(skill=>session.push({v,skill}))});index=0;current=null;show('study');renderCurrent()}
function senseiRecommendation(topic){const stats=topicStats(topic),weak=topicWeakestSkill(topic),labels={meaning:'meaning recall',listening:'listening',reading:'reading',picture:'picture memory',sentence:'sentence understanding',production:'active recall',kanji:'written Japanese'};if(stats.bossReady&&!stats.complete)return`You are ready to challenge the ${topic.title} boss.`;if(stats.introduced<stats.target)return`Continue ${topic.title}. I have chosen the next words and memory games.`;return`Your weakest skill in ${topic.title} is ${labels[weak]||weak}. Let’s train it.`}
...
function enrichVocabularyFromAnki(){
 ...
 vocab.forEach((word,originalIndex)=>{const record=byWord.get(word.word);word._originalOrder=originalIndex;if(!record)return;word._anki=record;word.topicId=record.topicId;word.topic=record.topic;word.topicOrder=record.topicOrder;word.contentOrder=record.contentOrder;word.exampleSentence=record.sentence||'';word.exampleSentenceRomaji=record.sentenceRomaji||'';word.exampleSentenceMeaning=record.sentenceMeaning||''});
 vocab.sort((a,b)=>Number(a.topicOrder??999)-Number(b.topicOrder??999)||Number(a.contentOrder??999999)-Number(b.contentOrder??999999)||Number(a._originalOrder)-Number(b._originalOrder));
}
function senseiBlock(message,compact=true){return `<aside class="sensei-guide ${compact?'compact':''}"><div class="sensei-avatar"><img src="media/profiles/master-base.webp?v=${APP_VERSION}" alt="Sensei" onerror="this.hidden=true;this.nextElementSibling.hidden=false"><span hidden>👴</span></div><div><strong>Sensei</strong><p>${esc(message)}</p></div></aside>`}
...
function renderJourney(){
 ...
 $('#topicAdventure').innerHTML=journeyTopics().map((topic,itemIndex)=>{const stats=topicStats(topic),unlockedTopic=topicUnlocked(itemIndex),current=itemIndex===currentTopicIndex(),status=stats.complete?'✓ Boss defeated':stats.bossReady?'Boss ready':unlockedTopic?'In progress':'🔒 Locked';if(!current)return `<article class="topic-region compact ${stats.complete?'complete':''} ${unlockedTopic?'unlocked':'locked'}"><button data-topic-launch="${esc(topic.id)}"${unlockedTopic?'':' disabled'}><span>${esc(topic.icon||'🗾')}</span><div><strong>${esc(topic.title)}</strong><small>${stats.introduced}/${stats.words.length} words · ${status}</small></div></button></article>`;return `<article class="topic-region current ${stats.complete?'complete':''}"><div class="topic-region-heading"><span>${esc(topic.icon||'🗾')}</span><div><small>Current region</small><strong>${esc(topic.title)}</strong><p>${esc(senseiRecommendation(topic))}</p></div></div><div class="topic-region-stats"><span><b>${stats.introduced}</b> introduced</span><span><b>${stats.tested}</b> practised</span><span><b>${stats.mastered}</b> mastered</span></div><i class="topic-progress"><b style="width:${stats.percent}%"></b></i><div class="topic-region-actions"><button class="primary" data-topic-launch="${esc(topic.id)}">Continue topic</button>${stats.bossReady&&!stats.complete?`<button data-topic-boss="${esc(topic.id)}">⚔️ Challenge boss</button>`:''}</div></article>`}).join('');
 document.querySelectorAll('[data-topic-launch]').forEach(button=>button.onclick=()=>startTopicSession(button.dataset.topicLaunch));document.querySelectorAll('[data-topic-boss]').forEach(button=>button.onclick=()=>startTopicBoss(button.dataset.topicBoss));
...
function startTopicBoss(topicId){const topic=journeyTopics().find(item=>item.id===topicId);if(!topic)return;const stats=topicStats(topic);if(!stats.bossReady){toast('Sensei will unlock the boss after more topic practice');return}const skills=['meaning','listening','reading','picture','sentence','production'];session=[];shuffle(topic.words.filter(wordIntroduced)).slice(0,Math.min(12,topic.words.length)).forEach((v,i)=>{let skill=skills[i%skills.length];if(skill==='picture'&&!memoryScenes[sceneKey(v)])skill='meaning';if(skill==='sentence'&&!v.sentence&&!v._anki?.sentence)skill='reading';session.push({v,skill,bossTopicId:topic.id})});meta.activeTopicBoss={topicId,correct:0,total:session.length};activityReturnScreen='journey';index=0;current=null;show('study');renderCurrent();toast(`${topic.icon||'⚔️'} ${topic.title} boss started`)}
...
function makeSession(chapterIndex=null){
 ...
 const queue=selected.map(v=>{
   const p=pFor(v.id);
   const unknownKana=unknownKanaFor(v).slice(0,2);let skills=p.stage===0?[...(unknownKana.length?['kanaUnlock']:[]),'firstEncounter','intro',...(ankiRecordFor(v)?.sentence?['example']:[]),'meaning','listening','reading','picture']:[chooseSkill(v)];
...
if(skill==='kanaUnlock'){const characters=unknownKanaFor(v).slice(0,2),character=characters[0],entry=kanaData.find(item=>item.kana===character),topic=topicForWord(v);if(!character){next();return}c.innerHTML=`${senseiBlock(`You will need ${character} to learn ${v.word} in ${topic.title}.`)}<div class="eyebrow">Character unlock</div><div class="kana-unlock-glyph" lang="ja">${esc(character)}</div><div class="kana-unlock-sound">${esc(entry?.romaji||'New Japanese sound')}</div><button id="kanaUnlockAudio" class="audio primary">🔊 Hear the sound</button><button id="kanaUnlockContinue" class="primary reveal">Use it in ${esc(v.word)} →</button>`;$('#kanaUnlockAudio').onclick=()=>entry&&playKana(entry);$('#kanaUnlockContinue').onclick=()=>{markKanaIntroduced(character);next()};if(entry&&settings.autoAudio)playKana(entry);return}
...
async function init(){
 ...
 try{
  [vocab,kanaData,mangaStories,conversations,theatreScenes,grammarLessons,componentData,memoryScenes,ankiContent,topicData]=await Promise.all([
 ...
 const versionCard=$('.version-card');if(versionCard){versionCard.querySelector('strong').textContent='Kaishi Quest v8.0.0';versionCard.querySelector('span').textContent='Integrated Journey';versionCard.querySelector('small').textContent='Sensei now links required kana, first encounters, mnemonic images and example sentences into one continuous learning flow.'}
