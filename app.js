...
function updateHome(){
 ...
 if($('#mangaBtn'))$('#mangaBtn').textContent=`📖 Manga Stories${mangaStories.length?` · ${mangaCompletedCount()}/${mangaStories.length}`:''}`;
...
function renderMangaLibrary(){
 ...
 $('#mangaStoryGrid').innerHTML=mangaStories.map(story=>{const state=mangaProgressFor(story.id),done=state.completed>0;return `<button class="manga-story-card" data-manga-story="${esc(story.id)}"><img src="${esc(story.image)}?v=${APP_VERSION}" alt="Manga page for ${esc(story.englishTitle)}"><span class="manga-story-level">Level ${story.difficulty} · ${mangaDifficulty(story.difficulty)}</span><strong lang="ja">${esc(story.title)}</strong><small>${esc(story.englishTitle)}</small><p>${esc(story.summary)}</p><b>${story.panels.length} panels${done?` · ✓ Complete · Best ${state.best}%`:''}</b></button>`}).join('');
 document.querySelectorAll('[data-manga-story]').forEach(button=>button.onclick=()=>startMangaStory(mangaStories.find(story=>story.id===button.dataset.mangaStory)));
}
...
function startMangaStory(story){
 if(!story)return;mangaStory=story;mangaPanelIndex=0;mangaAnswered=false;mangaQuestionAnswered=false;mangaRun={correct:0,total:0};const state=mangaProgressFor(story.id);state.attempts++;state.updatedAt=Date.now();save();$('#mangaLibrary').hidden=true;$('#mangaReader').hidden=false;renderMangaPanel();
}
...
async function init(){
 ...
 try{
  [vocab,kanaData,mangaStories,memoryScenes]=await Promise.all([
