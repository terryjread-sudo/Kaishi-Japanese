...
let activeJourneyMission=null,activeQuickStep=false,activeFirstLesson=false,kotobaEchoRun=null;
...
function renderKotobaEcho(){const run=kotobaEchoRun,card=$('#kotobaEchoCard');if(!run||!card)return;const word=run.queue[run.index];if(!word){if(run.phase==='first'&&run.missed.size){run.phase='retry';run.queue=run.firstRound.filter(item=>run.missed.has(item.id));run.index=0;renderKotobaEcho();return}finishKotobaEcho();return}const roundLabel=run.phase==='retry'?'Encore · missed words':'Main set',progress=`${run.index+1} / ${run.queue.length}`;card.dataset.effect=String((run.index%4)+1);card.innerHTML=`<section class="kotoba-echo-call"><div class="kotoba-echo-lights" aria-hidden="true"><i></i><i></i><i></i></div><div class="kotoba-echo-stage-head"><span>🎤 ${roundLabel}</span><b>${progress}</b></div><div class="kotoba-echo-equalizer" aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></div><span class="eyebrow">Your line</span><h2>${esc(word.meaning)}</h2><p>Say the Japanese word naturally. The answer appears after your attempt.</p><button id="kotobaEchoSpeak" class="primary" type="button">🎙️ Check my Japanese</button><p class="kotoba-echo-privacy">Browser recognition checks this attempt. Kaishi Quest does not store your voice or transcript.</p></section>`;$('#kotobaEchoSpeak').onclick=()=>{const coach=window.KaishiPronunciation;if(!coach?.recognitionAvailable){toast('Kotoba Echo needs browser speech recognition. Try pronunciation practice instead.');return}coach.open({phrase:word.word,reading:word.reading,meaning:word.meaning,chunks:[word.word],compact:true,onRecognitionResult:result=>{$('#pronunciationCoachDialog')?.close();if(result.retryable){renderKotobaEchoFeedback(word,'retry');return}if(result.matched){run.correct++;kotobaEchoStrengthen(word);renderKotobaEchoFeedback(word,'correct');return}if(run.phase==='first')run.missed.add(word.id);renderKotobaEchoFeedback(word,'wrong')}})}}
...
function startKotobaEcho(){activityReturnScreen='journey';const words=kotobaEchoWords();show('kotobaEcho');const card=$('#kotobaEchoCard');if(!window.KaishiPronunciation?.recognitionAvailable){const fallback=words[0];card.innerHTML=`<section class="kotoba-echo-summary"><span class="kotoba-echo-microphone" aria-hidden="true">🎤</span><span class="eyebrow">Speaking support</span><h2>Kotoba Echo needs browser recognition</h2><p>Your browser cannot check spoken Japanese here. You can still use private record-and-compare pronunciation practice.</p>${fallback?'<button id="kotobaEchoFallback" class="primary" type="button">Open pronunciation practice</button>':''}</section>`;$('#kotobaEchoFallback')?.addEventListener('click',()=>window.KaishiPronunciation?.open({phrase:fallback.word,reading:fallback.reading,meaning:fallback.meaning,chunks:[fallback.word],compact:true}));return}if(words.length<5&&!isAdminTestMode()){card.innerHTML=`<section class="kotoba-echo-summary"><span class="kotoba-echo-microphone" aria-hidden="true">🎤</span><span class="eyebrow">Warm up first</span><h2>Five familiar words unlock the stage</h2><p>Learn ${5-words.length} more audio-backed word${5-words.length===1?'':'s'}, then return for your first karaoke run.</p><button id="kotobaEchoNotReady" type="button">Return to Journey</button></section>`;$('#kotobaEchoNotReady').onclick=()=>openJourney('missions');return}const firstRound=[...words].sort((left,right)=>Number(pFor(left.id).skills.production.strength||0)-Number(pFor(right.id).skills.production.strength||0)||Math.random()-.5).slice(0,5);kotobaEchoRun={firstRound,queue:firstRound,index:0,phase:'first',missed:new Set(),correct:0};renderKotobaEcho()}
...
$('#kotobaEchoBack').onclick=()=>{kotobaEchoRun=null;openJourney('missions')};
