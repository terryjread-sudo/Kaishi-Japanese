...
function renderJourney(){
 ...
 $('#wordExpedition').innerHTML=Array.from({length:wordChapterCount()},(_,itemIndex)=>{const stats=chapterStats(itemIndex),unlockedChapter=chapterUnlocked(itemIndex),current=itemIndex===currentWordChapterIndex(),start=itemIndex*WORD_CHAPTER_SIZE+1,end=start+stats.words.length-1,name=WORD_CHAPTER_NAMES[itemIndex]||`Chapter ${itemIndex+1}`,status=stats.complete?'✓ Complete':unlockedChapter?'Available':'🔒 Locked';if(!current)return `<article class="word-chapter compact ${stats.complete?'complete':''} ${unlockedChapter?'unlocked':'locked'}"><button data-word-chapter="${itemIndex}"${unlockedChapter?'':' disabled'}><span class="chapter-number">Chapter ${itemIndex+1}</span><strong>${esc(name)}</strong><small>Words ${start}–${end} · ${stats.percent}% · ${status}</small></button></article>`;return `<article class="word-chapter current ${stats.complete?'complete':''} ${unlockedChapter?'unlocked':'locked'}"><img class="chapter-avatar" src="${pathAvatarSource()}" alt="Your Kaishi character in ${esc(name)}"><button data-word-chapter="${itemIndex}"><span class="chapter-number">Chapter ${itemIndex+1}</span><strong>${esc(name)}</strong><small>Words ${start}–${end}</small><div class="chapter-meter"><span><b style="width:${Math.min(100,stats.introduced/Math.max(1,stats.introTarget)*100)}%"></b></span><i>${stats.introduced}/${stats.introTarget} introduced</i></div><div class="chapter-meter review"><span><b style="width:${Math.min(100,stats.reviewed/Math.max(1,stats.reviewTarget)*100)}%"></b></span><i>${stats.reviewed}/${stats.reviewTarget} reviewed</i></div><em>${stats.complete?'✓ Chapter complete':'Continue chapter'}</em></button></article>`}).join('');
...
async function init(){
 ...
 const versionCard=$('.version-card');if(versionCard){versionCard.querySelector('strong').textContent='Kaishi Quest v7.2.0';versionCard.querySelector('span').textContent='Integrated Journey';versionCard.querySelector('small').textContent='Sensei now links required kana, first encounters, mnemonic images and example sentences into one continuous learning flow.'}
