...
const GARDEN_ITEMS=[{id:'smooth-stone',name:'River stone',icon:'🪨',cost:0,description:'A calm starting stone.'},{id:'moss-stone',name:'Moss stone',icon:'🌿',cost:80,description:'A soft green accent.'},{id:'bamboo',name:'Bamboo fountain',icon:'🎋',cost:160,description:'A quiet corner feature.'},{id:'lantern',name:'Stone lantern',icon:'🏮',cost:220,description:'A warm study marker.'},{id:'maple',name:'Maple tree',icon:'🍁',cost:300,description:'Colour earned through practice.'},{id:'water-basin',name:'Water basin',icon:'💧',cost:380,description:'A reflective garden centrepiece.'}];
...
let activeJourneyMission=null,activeQuickRake=false;
...
const META_DEFAULTS={lastStudy:'',streak:0,totalAnswers:0,totalCorrect:0,kanaAnswers:0,kanaCorrect:0,grammarAnswers:0,grammarCorrect:0,kanaProgress:{},grammarProgress:{},sentenceLabProgress:{lessons:{},saved:[],mistakes:[],totalAnswers:0,totalCorrect:0},mangaProgress:{},conversationProgress:{},theatreProgress:{},canDoAwards:[],pathUnlocks:[],pathVisits:{},pathOverrides:[],chapterOverrides:[],dailyJourneyRoute:null,dailyActivity:null,unlockNoticesSeen:[],unlockNoticesDismissed:[],activityPurchases:['vocabulary','kana','sentenceLab'],adventurePointsSpent:0,gardenApSpent:0,garden:{behind:0,lastChecked:'',quickWeek:'',quickUsed:0,owned:['smooth-stone'],placed:['smooth-stone'],cycles:0},activityModeLevels:{},karutaSessions:[],monsterVictories:[],totalMonsterVictories:0,streakRescue:null,activeCampaign:'journey',campaignProgress:{},updatedAt:0};
...
meta.activeCampaign=meta.activeCampaign||'journey';meta.campaignProgress=meta.campaignProgress||{};meta.activityPurchases=Array.isArray(meta.activityPurchases)?[...new Set(['vocabulary','kana','sentenceLab',...meta.activityPurchases])]:['vocabulary','kana','sentenceLab'];meta.activityModeLevels=meta.activityModeLevels||{};meta.adventurePointsSpent=Number(meta.adventurePointsSpent||0);meta.gardenApSpent=Number(meta.gardenApSpent||0);meta.garden={...META_DEFAULTS.garden,...(meta.garden||{})};meta.garden.owned=Array.isArray(meta.garden.owned)?[...new Set(['smooth-stone',...meta.garden.owned])]:['smooth-stone'];meta.garden.placed=Array.isArray(meta.garden.placed)?meta.garden.placed.filter(id=>meta.garden.owned.includes(id)):['smooth-stone'];
...
function saveMissionResume(){
 ...
 localStorage.setItem(profileStorageKey('kq-resumable-mission'),JSON.stringify({version:1,date:day(),remaining,activityReturnScreen,activeJourneyMission,activeQuickRake}));
}
...
function resumeSavedMission(){
 ...
 session=saved.session;index=0;current=null;activityReturnScreen=saved.activityReturnScreen||'journey';activeJourneyMission=saved.activeJourneyMission||null;activeQuickRake=Boolean(saved.activeQuickRake);
...
function gardenState(){const garden=meta.garden={...META_DEFAULTS.garden,...(meta.garden||{})};garden.behind=Math.max(0,Math.min(3,Number(garden.behind)||0));garden.owned=Array.isArray(garden.owned)?[...new Set(['smooth-stone',...garden.owned])]:['smooth-stone'];garden.placed=Array.isArray(garden.placed)?garden.placed.filter(id=>garden.owned.includes(id)):['smooth-stone'];const now=new Date(),monday=new Date(now.getFullYear(),now.getMonth(),now.getDate()-((now.getDay()+6)%7)),week=day(monday);if(garden.quickWeek!==week){garden.quickWeek=week;garden.quickUsed=0}garden.quickUsed=Math.max(0,Math.min(2,Number(garden.quickUsed)||0));return garden}
...
function breakGardenStreak(garden){if(Number(meta.streak||0)<=0){garden.behind=0;return}const lostAt=Date.now();meta.streakRescue={lostStreak:Number(meta.streak),lastStudyBeforeLoss:meta.lastStudy,lostAt,expiresAt:lostAt+7*86400000,attempted:false};meta.streak=0;garden.behind=0;garden.cycles=Number(garden.cycles||0)+1;garden.lastBrokenAt=day()}
function reconcileGarden(){const garden=gardenState(),today=day();if(!garden.lastChecked){const missed=Math.max(0,dateGap(meta.lastStudy,today)-1);garden.behind=Math.min(3,missed);garden.lastChecked=today;if(garden.behind>=3)breakGardenStreak(garden);return garden}if(garden.lastChecked===today)return garden;let cursor=new Date(`${garden.lastChecked}T12:00:00`),end=new Date(`${today}T12:00:00`);while(cursor<end){const date=day(cursor);if(date!==meta.lastStudy)garden.behind++;cursor.setDate(cursor.getDate()+1)}garden.lastChecked=today;if(garden.behind>=3)breakGardenStreak(garden);return garden}
function awardGardenDay(quick=false){const garden=reconcileGarden(),today=day(),gap=dateGap(meta.lastStudy,today);if(meta.lastStudy!==today){meta.streak=Number(meta.streak||0)>0&&gap<=3?Number(meta.streak)+1:1;meta.lastStudy=today}if(quick)garden.quickUsed=Math.min(2,garden.quickUsed+1);else if(garden.behind>0)garden.behind--;garden.lastChecked=today;return garden}
function recordMeaningfulActivity(source,amount=1){
 ...
 const testedForGoal=activity.quickRake?Math.max(0,activity.tested-Number(activity.quickStartTested||0)):activity.tested;
 const target=activity.quickRake?3:5;if(testedForGoal>=target&&!activity.qualified)activity.qualified=true;
 if(activity.qualified&&!activity.streakAwarded){
  const garden=awardGardenDay(Boolean(activity.quickRake));
  activity.streakAwarded=true;
  toast(activity.quickRake?'Quick Rake complete — your garden is cared for!':garden.behind?'Garden progress restored — keep going!':'A fresh pattern has been raked!');
 }
 return activity;
}
...
function detectLostStreak(){
 const before=Number(meta.streak||0);reconcileGarden();if(before!==Number(meta.streak||0))save(false);
}
...
window.KaishiGardenBridge={state:()=>{const garden=reconcileGarden(),activity=todayActivity(),target=activity.quickRake?3:5,tested=activity.quickRake?Math.max(0,Number(activity.tested||0)-Number(activity.quickStartTested||0)):Number(activity.tested||0);return{streak:Number(meta.streak||0),behind:garden.behind,quickUsed:garden.quickUsed,quickRemaining:Math.max(0,2-garden.quickUsed),owned:[...garden.owned],placed:[...garden.placed],cycles:Number(garden.cycles||0),todayTested:tested,todayTarget:target,qualified:Boolean(activity.qualified),quickActive:Boolean(activity.quickRake),availableAp:adventurePoints(),lifetimeAp:lifetimeXp()}},items:()=>GARDEN_ITEMS.map(item=>({...item})),buy:buyGardenItem,startQuick:startQuickRake,publicState:gardenPublicState,save:()=>save(),update:updateHome};
window.KaishiQuestCloudAdapter={
 snapshot:()=>isAdminTestMode()?null:{version:3,progress,meta,settings},
 restore:data=>{const localDailyReviewPlan=meta.dailyReviewPlan&&meta.dailyReviewPlan.date===day()?structuredClone(meta.dailyReviewPlan):null;progress=data?.progress||{};meta={...META_DEFAULTS,...(data?.meta||{})};meta.kanaProgress=meta.kanaProgress||{};meta.grammarProgress=meta.grammarProgress||{};meta.sentenceLabProgress={lessons:{},saved:[],mistakes:[],totalAnswers:0,totalCorrect:0,...(meta.sentenceLabProgress||{})};meta.mangaProgress=meta.mangaProgress||{};meta.conversationProgress=meta.conversationProgress||{};meta.theatreProgress=meta.theatreProgress||{};meta.pathUnlocks=Array.isArray(meta.pathUnlocks)?[...meta.pathUnlocks]:[];meta.pathVisits={...(meta.pathVisits||{})};meta.pathOverrides=Array.isArray(meta.pathOverrides)?[...meta.pathOverrides]:[];meta.chapterOverrides=Array.isArray(meta.chapterOverrides)?[...meta.chapterOverrides]:[];meta.gardenApSpent=Number(meta.gardenApSpent||0);gardenState();if(localDailyReviewPlan&&(!meta.dailyReviewPlan||meta.dailyReviewPlan.date!==day()||Number(localDailyReviewPlan.initialTotal||0)>=Number(meta.dailyReviewPlan.initialTotal||0)))meta.dailyReviewPlan=localDailyReviewPlan;settings={...defaults,...(data?.settings||{}),playMode:'journey'};save(false);if($('#pictureDifficulty')){$('#pictureDifficulty').value=settings.pictureDifficulty;$('#mnemonicStyle').value=settings.mnemonicStyle;$('#autoAudio').checked=settings.autoAudio;if($('#activityVillageMode'))$('#activityVillageMode').checked=settings.activityVillageMode!==false;if($('#activityVillageMode'))$('#activityVillageMode').checked=settings.activityVillageMode!==false}renderLearningBalanceSettings();updateHome();toast('Cloud progress restored')},
 stats:()=>{const mastered=Object.values(progress).filter(mastery).length,reviews=Number(meta.totalAnswers||0),correct=Number(meta.totalCorrect||0),monsters=Number(meta.totalMonsterVictories||meta.monsterVictories?.length||0),streak=Number(meta.streak||0);return{xp:Math.max(0,correct*10+mastered*50+monsters*100+streak*20),mastered,accuracy:reviews?Math.round(correct/reviews*100):0,reviews,monsters_defeated:monsters,streak}},garden:gardenPublicState,
...
function updateHome(){
 ...
 const streakTarget=activity.quickRake?3:5,streakTested=activity.quickRake?Math.max(0,Number(activity.tested||0)-Number(activity.quickStartTested||0)):Number(activity.tested||0);$('#streakActivity').textContent=activity.qualified?'Garden cared for today':`${Math.min(streakTested,streakTarget)} / ${streakTarget} tested answers`;$('#streakActivityFill').style.width=`${Math.min(100,streakTested/streakTarget*100)}%`;
 ...
 $('#streak').textContent=`${meta.streak} day${meta.streak===1?'':'s'} · Garden streak`;$('#summary').textContent=due?`${due} of today's reviews are ready.`:activity.qualified?'Today’s garden is cared for. Keep learning or enjoy a game.':`Reviews complete. Answer ${streakTarget} tested questions to tend today’s garden.`;
 renderSkillScores();updateKanaOverview();updateStreakRescue();window.KaishiCloud?.renderDashboardAvatar?.();renderJourneyHome();renderOwnerPathControls();window.KaishiGarden?.render?.()
}
...
function next(){if(activeQuickRake&&Math.max(0,todayActivity().tested-Number(todayActivity().quickStartTested||0))>=3){session=session.slice(0,3);index=session.length;renderCurrent();return}saveMissionResume();const completed=index+1;if(completed<session.length&&completed%CHECKPOINT_INTERVAL===0&&!pictureGameActive&&!karutaActive&&!battleActive){showMissionCheckpoint();return}index++;renderCurrent()}
function finishSession(){clearMissionResume();const quickCompleted=activeQuickRake;activeQuickRake=false;const completedJourneyMission=Boolean(activeJourneyMission)&&finishActiveJourneyMission();if(battleActive){showBattleSummary();return}if(karutaActive){showKarutaSummary();return}if(pictureGameActive){abortSession('games');toast('Game complete 🎉');return}save();updateHome();if(completedJourneyMission)renderJourney();const c=$('#card');$('#sessionCounter').textContent='Complete';$('#progressFill').style.width='100%';if(quickCompleted){c.innerHTML=`${senseiBlock('Quick Rake complete. Even a short, focused practice keeps your Japanese garden cared for.',true,'celebrating')}<div class="eyebrow">Garden cared for</div><h2>Your pattern continues.</h2><p>Three genuine review attempts protected today without turning practice into a chore.</p><div class="mission-complete-actions"><button id="finishMissionNow" class="primary">Return to my garden</button><button id="keepLearningMission">Keep learning</button></div>`;$('#finishMissionNow').onclick=()=>{show('home');window.KaishiGarden?.open?.()};$('#keepLearningMission').onclick=()=>{session=[];index=0;current=null;startTopicSession(currentTopic().id)};return}c.innerHTML=`${senseiBlock('Mandatory mission complete. Your progress is safely saved.')}<div class="eyebrow">Mission complete</div><h2>Great work — you reached today’s save point.</h2><p>You can finish now, or continue with another short optional mission.</p><div class="mission-complete-actions"><button id="finishMissionNow" class="primary">Finish for now</button><button id="keepLearningMission">Keep learning</button></div>`;$('#finishMissionNow').onclick=()=>{activityReturnScreen=activityReturnScreen==='journey'?'home':activityReturnScreen;returnToActivitySource('home');toast(todayActivity().qualified?'Mission complete — garden cared for 🎉':'Mission complete — progress saved')};$('#keepLearningMission').onclick=()=>{session=[];index=0;current=null;startTopicSession(currentTopic().id)}}
