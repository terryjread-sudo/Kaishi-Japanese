...
function render() {
  ...
  if (!target) return;
  host().show?.('gameHub');
  target.innerHTML = `<main class="game-hub"><header class="game-hub-heading"><span class="eyebrow">Games</span><h1>Choose a Japanese challenge</h1><p>Each game strengthens your Kaishi learning record.</p></header><div class="game-hub-grid"><button type="button" class="game-hub-card" data-games-checkpoint><span aria-hidden="true">🛂</span><strong>Kotoba Checkpoint</strong><small>Inspect documents, read Japanese, and decide who can pass.</small></button><button type="button" class="game-hub-card" data-device-repair-launch><span aria-hidden="true">📼</span><strong>Device Repair</strong><small>Restore a detailed cassette player with words you have already learned.</small></button></div><button type="button" class="game-hub-back" data-games-back>← Journey</button></main>`;
