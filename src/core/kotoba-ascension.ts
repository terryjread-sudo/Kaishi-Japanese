...
function renderCombat(state: AscensionRunState): void {
  ...
  target.innerHTML = frame(state, `<section class="ascension-battle-card"><div class="ascension-enemy"><span class="ascension-enemy-sprite">${state.enemy.boss ? '⛩' : state.enemy.elite ? '👹' : '🦝'}</span><div><span class="eyebrow">${state.enemy.boss ? `Final test · Phase ${state.enemy.phase}` : state.enemy.elite ? 'Elite encounter' : 'Encounter'}</span><h2>${escapeHtml(state.enemy.name)}</h2><p class="ascension-intent"><b>${intentLabel(state)}</b></p><i><b style="width:${state.enemy.hp / state.enemy.maxHp * 100}%"></b></i></div></div><p class="ascension-message">${escapeHtml(state.pendingMessage)}</p><div class="ascension-combat-status"><span>Selected chain: ${selectedCost || 0} energy</span><span>Guard: ${state.block}</span><span>Draw pile: ${state.drawPile.length}</span></div><div class="ascension-hand" aria-label="Choose cards">${cards}</div><div class="ascension-battle-actions"><button type="button" class="primary" data-ascension-play ${selectedCost > state.resources.energy || !selected.size ? 'disabled' : ''}>Use chain</button><button type="button" data-ascension-audio>🔊 Hear a card</button></div><p class="ascension-tip">Try noun + を + verb for an attack, or noun + で + verb to defend. A warded enemy needs a stronger chain.</p></section>`);
  bindExit(target);
